...
export default function OverwatchRandomV2Page() {
  ...
  return (
    <>
      <Head>
        <title>오버워치 랜덤뽑기 V2 | 장지수 팬 아카이브</title>
        <meta name="description" content="가독성 위주로 재구성한 오버워치 랜덤뽑기 V2 프리뷰" />
      </Head>
      <div className="min-h-screen bg-[#05070c] text-white">
        ...
        <header className="sticky top-0 z-40 border-b border-white/10 bg-black/70 backdrop-blur-xl">
          <div className="mx-auto flex max-w-[108rem] items-center justify-between gap-4 px-5 py-4 lg:px-8">
            ...
            <nav className="flex flex-wrap items-center justify-end gap-3">
              ...
              <a href="/utility/overwatch-random" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2.5 text-base font-medium text-white/80 transition hover:bg-white/10">기존 버전</a>
            </nav>
          </div>
        </header>
        ...
        <main className="relative mx-auto max-w-[108rem] px-5 py-8 lg:px-8">
          <section className="rounded-[34px] border border-white/10 bg-[linear-gradient(145deg,rgba(23,28,40,0.98),rgba(10,12,18,0.98))] p-8 shadow-2xl shadow-black/30 lg:p-10">
            <div className="text-sm font-bold tracking-[0.42em] text-orange-200/58">UTILITY V2 PREVIEW</div>
            <div className="mt-4 text-[38px] font-black tracking-tight text-white sm:text-[52px]">오버워치 랜덤뽑기 레이아웃 개편안</div>
            <p className="mt-4 max-w-4xl text-base leading-8 text-white/68">등록 / 설정 / 결과를 한눈에 보이도록 다시 정리한 프리뷰입니다. 기존보다 팀 결과판 대비를 더 높이고, 포지션별 색상 구분과 액션 버튼 우선순위를 더 분명하게 잡았습니다.</p>
          </section>
          ...
          <section className="mt-8 grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
              ...
              <SectionCard title="요약" desc="전체 인원과 필요 슬롯을 먼저 확인하세요.">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-4">
                    <div className="text-sm text-white/48">등록 인원</div>
                    <div className="mt-2 text-[28px] font-black text-white">{participants.length}</div>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-4">
                    <div className="text-sm text-white/48">총 슬롯</div>
                    <div className="mt-2 text-[28px] font-black text-white">{totalSlots}</div>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {Object.entries(POSITION_META).map(([key, meta]) => (
                    <div key={key} className={`rounded-full border px-3 py-2 text-sm font-semibold ${meta.chip}`}>{meta.icon} {meta.label} {grouped[key].length}명</div>
                  ))}
                </div>
              </SectionCard>
            </div>
            ...
            <div className="space-y-6">
              <SectionCard
                title="설정 / 컨트롤"
                desc="중요 버튼만 상단에 모아서 방송 중에도 바로 조작할 수 있게 구성했습니다."
                action={<div className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-white/62">V2 PREVIEW</div>}
              >
                <div className="grid gap-4 xl:grid-cols-[210px_210px_minmax(0,1fr)]">
                  <label className="rounded-2xl border border-white/10 bg-[#121926] px-4 py-4">
                    <div className="text-sm text-white/48">팀 수</div>
                    <select value={teamCount} onChange={(e) => handleTeamCount(e.target.value)} className="mt-2 w-full bg-transparent text-[24px] font-black text-white outline-none">
                      {Array.from({ length: 9 }, (_, index) => index + 2).map((count) => <option key={count} value={count}>{count}팀</option>)}
                    </select>
                  </label>
                  <label className="rounded-2xl border border-white/10 bg-[#121926] px-4 py-4">
                    <div className="text-sm text-white/48">게임 방식</div>
                    <select value={mode} onChange={(e) => setMode(e.target.value)} className="mt-2 w-full bg-transparent text-[24px] font-black text-white outline-none">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <a href="/utility/overwatch-random" className="rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-4 text-center text-base font-bold text-white/72 transition hover:bg-white/[0.09]">기존 페이지</a>
                  </div>
                </div>
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  {Array.from({ length: teamCount }, (_, index) => (
                    <input key={`captain-${index}`} value={captainInputs[index] || ''} onChange={(e) => setCaptainInputs((prev) => { const next = [...prev]; next[index] = e.target.value; return next; })} placeholder={`팀장 이름 ${index + 1}`} className="w-full rounded-2xl border border-white/10 bg-[#121926] px-4 py-3 text-base text-white outline-none placeholder:text-white/28 focus:border-cyan-300/35" />
                  ))}
                </div>
              </SectionCard>

              <SectionCard title="포지션 보드" desc="색 대비를 더 올리고 보드를 나눠서 현재 분포가 바로 보이도록 구성했습니다.">
