...
function UtilityCard({
  ...
  if (disabled) {
    return <div aria-disabled="true" className={baseClassName}>{content}</div>;
  }
      ...
      <div className="min-h-screen bg-[#05070c] text-white">
        ...
        <main className="relative mx-auto max-w-7xl px-5 py-8 lg:px-8">
          ...
          <section className="mt-8 grid gap-6 xl:grid-cols-3">
            <UtilityCard href="/utility/overwatch-random" title="오버워치 랜덤뽑기" description="팀 수 2~10팀, 5:5·6:6 전환, 포지션 등록, 팀장 순서 추첨, 드래그 이동까지 포함한 방송용 팀 편성 툴" label="실행하기" preview={<OverwatchPreview />} />
            <UtilityCard href="/utility/soop-funding-memo" title="SOOP 펀딩 자동 메모장" description="스트리머 전용 SOOP 파트너 프로그램입니다. 별풍선 후원을 핀볼 복붙용 메모로 자동 변환하고, 후원자 순위까지 한 화면에서 관리합니다." label="스트리머 전용 열기" status="SOOP 파트너" accent="soop" preview={<SoopPreview />} />
            <UtilityCard href="/utility/soop-funding-new" title="SOOP 펀딩 New" description="SOOP 로그인과 토큰 없이 스트리머 아이디 기준으로 후원자 닉네임과 개수만 자동 수신하는 새 버전입니다." label="New 버전 열기" status="SSE 수신" accent="soop" preview={<SoopPreview />} />
          </section>
        </main>
      </div>
    </>
