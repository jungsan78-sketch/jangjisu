...
function OverwatchPreview() {
  return (
    <div className="relative flex w-full items-center justify-center select-none">
      ...
      <img src="/logos/overwatch.png" alt="오버워치" className="relative h-[96px] w-[96px] object-contain drop-shadow-[0_10px_22px_rgba(0,0,0,0.28)]" />
    </div>
  );
}
...
function SoopPreview() {
  return (
    <div className="relative flex w-full items-center justify-center select-none">
      ...
      <img src="/logos/SOOP.png" alt="SOOP" className="relative max-h-[82px] w-auto max-w-[260px] object-contain drop-shadow-[0_10px_22px_rgba(0,0,0,0.26)]" />
    </div>
  );
}

function UtilityCard({ href = '', title, description, label, eyebrow = '', accent = 'orange', preview = null, status = '', disabled = false }) {
  const theme = accent === 'soop'
    ? {
        border: disabled ? 'border-cyan-300/18' : 'hover:border-cyan-300/45 hover:shadow-[0_28px_70px_rgba(14,123,255,0.20)]',
    ...
    : {
        border: disabled ? 'border-orange-300/18' : 'hover:border-orange-300/35 hover:shadow-[0_24px_50px_rgba(249,115,22,0.14)]',
  ...
  if (disabled) return <div className={baseClassName}>{content}</div>;
...
export default function UtilityHomePage() {
  return (
    <>
      ...
      <main className="mt-8 grid gap-6 xl:grid-cols-3">
        <UtilityCard href="/utility/overwatch-random" title="오버워치 랜덤뽑기" description="팀 편성 툴" label="실행" preview={<OverwatchPreview />} />
        <UtilityCard href="/utility/soop-funding-memo" title="SOOP 펀딩 자동 메모장" description="기존 버전" label="열기" accent="soop" preview={<SoopPreview />} />
        <UtilityCard href="/utility/soop-funding-new" title="SOOP 펀딩 New" description="hoxy relay 기반" label="열기" accent="soop" preview={<SoopPreview />} />
        <UtilityCard href="/utility/soop-funding-new2" title="SOOP 펀딩 New2" description="완전 독립 수신기 테스트 버전" label="열기" accent="soop" preview={<SoopPreview />} />
      </main>
    </>
  );
}
