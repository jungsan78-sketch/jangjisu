...
const FIELD_CLASS = 'h-11 rounded-xl bg-slate-400/10 px-3 text-sm font-bold text-white shadow-[inset_0_1px_0_rgba(180,210,230,0.07)] outline-none placeholder:text-white/34 focus:ring-1 focus:ring-cyan-300/24';
...
function getRoleTemplate(mode) {
  return mode === '6v6' ? ['탱커1', '탱커2', '딜러1', '딜러2', '힐러1', '힐러2'] : ['탱커', '딜러1', '딜러2', '힐러1', '힐러2'];
}
...
function CustomDropdown({ value, options, onChange, className = '' }) {
  ...
  return (
    <div className={`relative ${className}`}>
      <button type="button" onClick={() => setOpen((prev) => !prev)} onBlur={() => setTimeout(() => setOpen(false), 120)} className="flex h-11 w-full items-center justify-between gap-2 rounded-xl bg-[#0d1523] px-3 text-sm font-black text-white shadow-[inset_0_1px_0_rgba(125,183,219,0.08)] transition hover:bg-[#121c2d] hover:ring-1 hover:ring-cyan-300/18">
      ...
      {open ? (
        <div className="absolute left-0 top-[calc(100%+8px)] z-[70] w-full overflow-hidden rounded-xl bg-[#070d18] p-1 shadow-[inset_0_1px_0_rgba(125,183,219,0.08),0_18px_42px_rgba(0,0,0,0.45)]">
          {options.map((option) => (
...
function MoveMenu({ target, teams, currentPlayer, currentTeam, currentLocked, onClose, onMoveToLobby, onMoveToTeam, onSwap, onToggleLock }) {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/64 px-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[86vh] w-full max-w-[520px] overflow-y-auto rounded-[28px] border border-white/12 bg-[#090e18] p-5 shadow-[0_28px_80px_rgba(0,0,0,0.55)]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 grid gap-2">
          ...
          {teams.map((team) => <button key={team.teamNo} onClick={() => onMoveToTeam(team.teamNo)} className="rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-3 text-left text-sm font-black text-white hover:bg-white/10">TEAM {team.teamNo}로 이동</button>)}
        </div>
        {otherPlayers.length ? <><div className="mt-5 text-sm font-black text-white/60">교체</div><div className="mt-2 grid max-h-[260px] gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{otherPlayers.map((player) => <button key={player.id} onClick={() => onSwap(player)} className="rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 text-left text-sm font-black text-white/86 hover:bg-white/10">TEAM {player.teamNo} · {player.name}</button>)}</div></> : null}
      </div>
    </div>
  );
}

function OverwatchRandomContent() {
  ...
  const getProfile = (name) => {
    const saved = participants.find((item) => item.name === name);
    return profileMap[normalizeName(name)] || (saved?.profileImage ? { nickname: name, profileImage: saved.profileImage } : null);
  };
  ...
  const addSuggestedParticipant = (candidate) => {
    ...
    if (candidate.profileImage) setProfileMap((prev) => ({ ...prev, [normalizeName(candidate.nickname)]: { nickname: candidate.nickname, profileImage: candidate.profileImage } }));
    setNameInput('');
  };
  ...
  const moveToPosition = (item, nextPosition) => {
    if (!item || !POSITION_META[nextPosition]) return;
    setParticipants((prev) => prev.map((candidate) => candidate.id === item.id ? { ...candidate, position: nextPosition } : candidate));
  ...
  return (
    <><Head><title>오버워치 랜덤뽑기 | 장지수 팬 아카이브</title><meta name="description" content="방송용 오버워치 랜덤 팀 편성 툴" /></Head><div className="min-h-screen bg-[#05070c] text-white"><div className="pointer-events-none fixed inset-0 overflow-hidden"><div className="absolute -top-24 left-[-90px] h-80 w-80 rounded-full bg-sky-500/10 blur-3xl" /><div className="absolute right-[-80px] top-16 h-96 w-96 rounded-full bg-orange-500/10 blur-3xl" /><div className="absolute bottom-0 left-1/2 h-72 w-[36rem] -translate-x-1/2 rounded-full bg-blue-500/8 blur-3xl" /></div><header className="sticky top-0 z-40 border-b border-white/10 bg-black/72 backdrop-blur-xl"><div className="mx-auto flex max-w-[118rem] items-center justify-between gap-4 px-5 py-4 lg:px-8"><a href="/" className="block h-12 w-12 overflow-hidden rounded-full border border-white/10 transition hover:scale-105"><img src="/site-icon.png" alt="SOU" className="h-full w-full object-cover" /></a><div className="text-[26px] font-black tracking-tight text-white">오버워치 랜덤뽑기</div><a href="/utility" className="rounded-full border border-cyan-300/28 bg-cyan-300/12 px-4 py-2 text-sm font-bold text-cyan-100 transition hover:bg-cyan-300/18">유틸리티</a></div></header><main className="relative mx-auto max-w-[118rem] px-5 py-6 lg:px-8"><section className="mb-5 rounded-[26px] bg-[linear-gradient(180deg,rgba(20,34,52,0.94),rgba(9,17,29,0.96))] p-4 shadow-[inset_0_1px_0_rgba(125,183,219,0.08),0_18px_50px_rgba(0,0,0,0.24)]"><div className="flex flex-wrap items-center gap-3"><StreamerAutocomplete value={nameInput} onChange={setNameInput} onSelect={addSuggestedParticipant} onSubmit={addParticipant} placeholder="스트리머 이름 검색..." className="w-[260px]" inputClassName={FIELD_CLASS} /><button onClick={addParticipant} className="h-11 rounded-xl bg-cyan-400/16 px-5 text-sm font-black text-cyan-50 shadow-[inset_0_1px_0_rgba(160,235,248,0.12),0_8px_22px_rgba(6,182,212,0.12)] transition hover:bg-cyan-400/24">추가</button><div className="mx-1 h-8 w-px bg-white/10" /><button onClick={() => setMode('5v5')} className={`h-10 rounded-xl px-4 text-sm font-black shadow-[inset_0_1px_0_rgba(180,210,230,0.07)] ${mode === '5v5' ? 'bg-orange-400/18 text-orange-50 ring-1 ring-orange-300/22' : 'bg-slate-400/10 text-slate-100/58 shadow-[inset_0_1px_0_rgba(180,210,230,0.06)]'}`}>5 vs 5</button><button onClick={() => setMode('6v6')} className={`h-10 rounded-xl px-4 text-sm font-black shadow-[inset_0_1px_0_rgba(180,210,230,0.07)] ${mode === '6v6' ? 'bg-orange-400/18 text-orange-50 ring-1 ring-orange-300/22' : 'bg-slate-400/10 text-slate-100/58 shadow-[inset_0_1px_0_rgba(180,210,230,0.06)]'}`}>6 vs 6</button><CustomDropdown value={teamCount} options={Array.from({ length: 9 }, (_, i) => ({ value: i + 2, label: `${i + 2}팀`, icon: '🎲' }))} onChange={setTeamCount} className="w-[104px]" /><button onClick={shuffleTeams} className="h-11 rounded-xl border border-orange-300/45 bg-orange-500 px-5 text-sm font-black text-white shadow-[0_12px_24px_rgba(249,115,22,0.20)] transition hover:brightness-110">랜덤 섞기</button><button onClick={resetTeams} className="h-11 rounded-xl bg-slate-400/10 px-4 text-sm font-bold text-slate-100/72 shadow-[inset_0_1px_0_rgba(180,210,230,0.08)] transition hover:bg-slate-300/15">팀 초기화</button><button onClick={copyResult} className="h-11 rounded-xl border border-cyan-300/28 bg-cyan-300/12 px-4 text-sm font-bold text-cyan-100 transition hover:bg-cyan-300/18">결과 복사</button><button onClick={() => setCurrentMap(MAP_POOL[Math.floor(Math.random() * MAP_POOL.length)])} className="h-11 rounded-xl border border-violet-300/28 bg-violet-300/12 px-4 text-sm font-bold text-violet-100 transition hover:bg-violet-300/18">맵 뽑기</button><button onClick={clearAll} className="ml-auto h-11 rounded-xl border border-rose-300/24 bg-rose-300/10 px-4 text-sm font-bold text-rose-100/80 transition hover:bg-rose-300/16">전체삭제</button></div><div className="mt-3 flex flex-wrap items-center gap-3 text-sm font-bold text-white/55"><span>등록 {participants.length}명</span><span>대기 {lobbyPlayers.length}명</span><span>배정 {assignedNames.size}명</span><span>잠금 {Object.values(slotLocks).filter(Boolean).length}칸</span>{currentMap ? <span className="rounded-full border border-violet-300/25 bg-violet-300/10 px-3 py-1 text-violet-100">현재 맵: {currentMap}</span> : null}</div></section><div className="mb-5 flex items-center justify-center rounded-[20px] bg-[linear-gradient(90deg,rgba(14,165,233,0.16),rgba(249,115,22,0.14))] px-5 py-4 text-center shadow-[inset_0_1px_0_rgba(153,216,241,0.11),0_10px_30px_rgba(0,0,0,0.18)]"><p className="text-sm font-black tracking-[-0.01em] text-cyan-50 sm:text-base">카드를 마우스로 드래그해 팀 배정과 포지션 변경이 가능합니다.</p></div><section className="grid gap-5 xl:grid-cols-[minmax(520px,0.95fr)_minmax(680px,1.35fr)]"><section className="min-h-[700px] rounded-[30px] bg-[linear-gradient(180deg,rgba(17,30,47,0.9),rgba(8,15,25,0.94))] p-5 shadow-[inset_0_1px_0_rgba(125,183,219,0.07),0_18px_50px_rgba(0,0,0,0.26)]"><div className="flex items-end justify-between gap-4"><div><div className="text-xs font-black tracking-[0.32em] text-cyan-100/45">WAITING ROOM</div><h2 className="mt-2 text-[34px] font-black tracking-tight text-white">대기실</h2></div><div className="rounded-full border border-white/10 bg-white/[0.055] px-4 py-2 text-sm font-black text-white/65">{lobbyPlayers.length}명</div></div><div className="mt-5 rounded-[26px] bg-[linear-gradient(180deg,rgba(11,22,37,0.82),rgba(6,13,23,0.72))] p-4 shadow-[inset_0_1px_0_rgba(125,183,219,0.055)]"><div className="grid gap-4">{['random','tank','dps','support'].map((type) => { const list = lobbyPlayers.filter((item) => item.position === type); const meta = POSITION_META[type]; return <div key={type} className="rounded-[24px] bg-[linear-gradient(180deg,rgba(10,20,34,0.86),rgba(6,12,22,0.72))] p-4 shadow-[inset_0_1px_0_rgba(125,183,219,0.055),0_10px_28px_rgba(0,0,0,0.16)] transition" onDragOver={(e) => e.preventDefault()} onDrop={() => { if (dragState?.item) { moveToPosition(dragState.item, type); setDragState(null); } }}><div className="mb-3 flex items-center justify-between"><div className="text-base font-black text-white">{meta.icon} {meta.label}</div><div className="text-sm font-bold text-white/40">{list.length}명</div></div><div className="flex min-h-[154px] flex-wrap gap-3 rounded-[20px] bg-black/10 p-3">{list.length ? list.map((item) => <PlayerCard key={item.id} item={item} profile={getProfile(item.name)} draggable onDragStart={() => setDragState({ item })} onDragEnd={() => setDragState(null)} onClick={() => setMoveTarget({ item })} onRemove={removeParticipant} />) : <div className="flex min-h-[130px] w-full items-center justify-center rounded-[20px] bg-white/[0.018] text-sm font-bold text-white/26">여기로 드래그해 포지션 변경</div>}</div></div>; })}</div></div></section><section className="rounded-[30px] bg-[linear-gradient(180deg,rgba(17,30,47,0.9),rgba(8,15,25,0.94))] p-5 shadow-[inset_0_1px_0_rgba(125,183,219,0.07),0_18px_50px_rgba(0,0,0,0.26)]"><div className="flex items-end justify-between gap-4"><div><div className="text-xs font-black tracking-[0.32em] text-orange-100/45">TEAM BOARD</div><h2 className="mt-2 text-[34px] font-black tracking-tight text-white">팀 결과판</h2></div><div className="rounded-full border border-white/10 bg-white/[0.055] px-4 py-2 text-sm font-black text-white/65">{teamCount}팀 · {mode}</div></div><div className="mt-5 grid gap-4 2xl:grid-cols-2">{teams.map((team) => { const theme = TEAM_ACCENTS[(team.teamNo - 1) % TEAM_ACCENTS.length]; return <div key={team.teamNo} className={`relative overflow-hidden rounded-[26px] bg-[#0b111d] p-4 shadow-[inset_0_1px_0_rgba(125,183,219,0.08),0_14px_36px_rgba(0,0,0,0.25)]`}><div className={`pointer-events-none absolute inset-x-0 top-0 h-24 ${theme.glow}`} /><div className="relative flex items-center justify-between gap-3 pb-3"><div className="text-[24px] font-black text-white">TEAM {team.teamNo}</div><div className={`rounded-full border px-3 py-1 text-xs font-black ${theme.badge}`}>{team.players.length}/{roles.length}</div></div><div className="relative mt-4 flex min-h-[286px] flex-wrap content-start gap-3">{roles.map((role) => { const slotKey = `${team.teamNo}-${role}`; const name = assignments[slotKey]; const item = name ? participants.find((p) => p.name === name) : null; return item ? <PlayerCard key={slotKey} item={item} profile={getProfile(item.name)} size="small" locked={Boolean(slotLocks[slotKey])} onToggleLock={() => toggleSlotLock(slotKey)} draggable onDragStart={() => setDragState({ item })} onDragEnd={() => setDragState(null)} onClick={() => setMoveTarget({ item })} /> : <EmptySlot key={slotKey} role={role} active={Boolean(dragState) && !slotLocks[slotKey]} onDrop={() => handleDropToSlot(team.teamNo, role)} />; })}</div></div>; })}</div></section></section></main><MoveMenu target={moveTarget} teams={teams} currentPlayer={currentPlayer} currentTeam={currentTeam} currentLocked={currentLocked} onClose={() => setMoveTarget(null)} onMoveToLobby={() => { moveToLobby(moveTarget.item); setMoveTarget(null); }} onMoveToTeam={(teamNo) => { moveToTeam(moveTarget.item, teamNo); setMoveTarget(null); }} onSwap={(player) => { swapPlayers(moveTarget.item, player); setMoveTarget(null); }} onToggleLock={() => { if (currentPlayer?.slotKey) toggleSlotLock(currentPlayer.slotKey); }} /></div></>
  );
}
