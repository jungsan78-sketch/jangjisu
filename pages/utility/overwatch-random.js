...
const STORAGE_KEY = 'sou:overwatch-random:v3';

const POSITION_META = {
  tank: { label: '탱커', icon: '🛡️', accent: 'sky', ring: 'border-sky-300/30', badge: 'border-sky-300/35 bg-sky-300/12 text-sky-100' },
  dps: { label: '딜러', icon: '⚔️', accent: 'orange', ring: 'border-orange-300/30', badge: 'border-orange-300/35 bg-orange-300/12 text-orange-100' },
  support: { label: '힐러', icon: '💚', accent: 'emerald', ring: 'border-emerald-300/30', badge: 'border-emerald-300/35 bg-emerald-300/12 text-emerald-100' },
  random: { label: '랜덤', icon: '❓', accent: 'violet', ring: 'border-violet-300/30', badge: 'border-violet-300/35 bg-violet-300/12 text-violet-100' },
};
...
function PlayerCard({ item, profile, size = 'large', faded = false, draggable = false, onDragStart, onDragEnd, onClick, onRemove }) {
  ...
  return (
    <button
      ...
      onDragEnd={onDragEnd}
      onClick={onClick}
      className={`group relative flex ${cardSize} flex-col items-center justify-start rounded-[22px] border ${meta.ring} bg-[linear-gradient(180deg,rgba(255,255,255,0.065),rgba(255,255,255,0.025))] text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_12px_28px_rgba(0,0,0,0.22)] transition hover:-translate-y-1 hover:border-white/24 hover:bg-white/[0.075] ${faded ? 'opacity-38 saturate-50' : ''} ${draggable ? 'cursor-grab active:cursor-grabbing' : 'cursor-pointer'}`}
    >
      ...
      <div className={`mt-2 rounded-full border px-2.5 py-1 text-[11px] font-black ${meta.badge}`}>{meta.icon} {meta.label}</div>
    </button>
  );
}
...
function EmptySlot({ role, onDrop, active }) {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`flex w-[112px] min-h-[122px] flex-col items-center justify-center rounded-[22px] border border-dashed ${active ? 'border-cyan-300/50 bg-cyan-300/10' : 'border-white/10 bg-white/[0.025]'} transition`}
    >
      <div className="h-14 w-14 rounded-full border border-white/8 bg-white/[0.035]" />
      <div className="mt-2 text-[11px] font-black text-white/34">{role}</div>
...
export default function OverwatchRandomPage() {
  ...
  const teams = useMemo(() => Array.from({ length: teamCount }, (_, i) => {
    ...
    const players = roles.map((role) => {
      const name = assignments[`${teamNo}-${role}`];
  ...
  useEffect(() => {
    if (!hydrated) return;
    writeSavedState({ teamCount, mode, participants, assignments, captains, currentMap });
  }, [hydrated, teamCount, mode, participants, assignments, captains, currentMap]);
  ...
  const removeParticipant = (name) => {
    ...
    setCaptains((prev) => Object.fromEntries(Object.entries(prev).map(([key, value]) => [key, value === name ? '' : value])));
  };
  ...
  const moveToLobby = (item) => {
    ...
    setCaptains((prev) => Object.fromEntries(Object.entries(prev).map(([key, value]) => [key, value === item.name ? '' : value])));
  };
  ...
  const shuffleTeams = () => {
    ...
    for (let teamNo = 1; teamNo <= teamCount; teamNo += 1) {
      roles.forEach((role) => {
        next[`${teamNo}-${role}`] = pool[index]?.name || '';
        index += 1;
      });
    }
    setAssignments(next);
  ...
  const resetTeams = () => { setAssignments({}); setCaptains({}); };
  const clearAll = () => {
    setParticipants([]); setAssignments({}); setCaptains({}); setCurrentMap('');
    if (typeof window !== 'undefined') window.localStorage.removeItem(STORAGE_KEY);
  };

  const copyResult = async () => {
    const text = teams.map((team) => {
      const captain = captains[team.teamNo] ? ` (팀장: ${captains[team.teamNo]})` : '';
      const players = roles.map((role) => `${role}: ${assignments[`${team.teamNo}-${role}`] || '-'}`).join('\n');
      return `TEAM ${team.teamNo}${captain}\n${players}`;
    }).join('\n\n');
  ...
  const handleDropToSlot = (teamNo, role) => {
    ...
    moveToSlot(dragState.item, `${teamNo}-${role}`);
    setDragState(null);
  };
        ...
        <main className="relative mx-auto max-w-[118rem] px-5 py-6 lg:px-8">
          <section className="mb-5 rounded-[26px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.018))] p-4 shadow-[0_18px_50px_rgba(0,0,0,0.24)]">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm font-bold text-white/55">
              <span>등록 {participants.length}명</span><span>대기 {lobbyPlayers.length}명</span><span>배정 {assignedNames.size}명</span>{currentMap ? <span className="rounded-full border border-violet-300/25 bg-violet-300/10 px-3 py-1 text-violet-100">현재 맵: {currentMap}</span> : null}
            </div>
          </section>
              ...
              <div className="mt-5 rounded-[24px] border border-white/8 bg-black/18 p-4">
                <div className="grid gap-4">
                  {['tank','dps','support','random'].map((type) => {
                    ...
                    return (
                      <div key={type} className="rounded-[22px] border border-white/8 bg-white/[0.025] p-4" onDragOver={(e) => e.preventDefault()} onDrop={() => { if (dragState?.item) { moveToLobby(dragState.item); setDragState(null); } }}>
                        ...
                        <div className="flex min-h-[154px] flex-wrap gap-3">
                          {list.length ? list.map((item) => <PlayerCard key={item.id} item={item} profile={getProfile(item.name)} draggable onDragStart={() => setDragState({ item })} onDragEnd={() => setDragState(null)} onClick={() => setMoveTarget({ item })} onRemove={removeParticipant} />) : <div className="flex min-h-[130px] w-full items-center justify-center rounded-[20px] border border-dashed border-white/8 bg-white/[0.018] text-sm font-bold text-white/28">대기중인 인원이 없습니다.</div>}
                        </div>
                      </div>
                    );
                  })}
                  ...
                  return (
                    <div key={team.teamNo} className={`relative overflow-hidden rounded-[26px] border ${theme.border} bg-[#0b111d] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_12px_32px_rgba(0,0,0,0.24)]`}>
                      ...
                      <div className="relative flex items-center justify-between gap-3 border-b border-white/8 pb-3">
                        ...
                        <select value={captains[team.teamNo] || ''} onChange={(e) => setCaptains((prev) => ({ ...prev, [team.teamNo]: e.target.value }))} className="h-9 rounded-xl border border-white/10 bg-black/22 px-3 text-xs font-black text-white outline-none">
                          <option value="">팀장 선택</option>
                          {team.players.map((player) => <option key={player.id} value={player.name}>{player.name}</option>)}
                        </select>
                      </div>
                        ...
                        {roles.map((role) => {
                          ...
                          return item ? (
                            <div key={slotKey} className="relative">
                              {captains[team.teamNo] === item.name ? <div className="absolute -top-2 left-1/2 z-10 -translate-x-1/2 rounded-full border border-amber-200/35 bg-amber-300/18 px-2 py-0.5 text-[10px] font-black text-amber-100">팀장</div> : null}
                              <PlayerCard item={item} profile={getProfile(item.name)} size="small" draggable onDragStart={() => setDragState({ item })} onDragEnd={() => setDragState(null)} onClick={() => setMoveTarget({ item })} />
                            </div>
                          ) : <EmptySlot key={slotKey} role={role} active={Boolean(dragState)} onDrop={() => handleDropToSlot(team.teamNo, role)} />;
                        })}
                      </div>
                    </div>
                  );
