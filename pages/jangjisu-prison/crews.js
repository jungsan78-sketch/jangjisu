...
function StatPill({ label, crewCount, memberCount }) {
  return <div className="group relative overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.085),rgba(255,255,255,0.026))] px-6 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07),0_18px_42px_rgba(0,0,0,0.22)] transition duration-300 hover:-translate-y-0.5 hover:border-cyan-200/18 hover:shadow-[0_22px_54px_rgba(0,0,0,0.30)]"><div className="pointer-events-none absolute inset-x-0 top-0 h-20 bg-[radial-gradient(circle_at_top_left,rgba(103,232,249,0.13),transparent_58%)] opacity-70 transition group-hover:opacity-100" /><div className="relative text-[15px] font-black tracking-[0.24em] text-white/52">{label}</div><div className="relative mt-1 text-[30px] font-black tracking-tight text-white sm:text-[34px]">{crewCount}개 크루 · {memberCount}명</div></div>;
}
...
function SoopButton({ stationUrl, large = false }) {
  const baseClass = `${large ? 'h-12 w-16' : 'h-11 w-14'} inline-flex items-center justify-center rounded-2xl border px-2 transition duration-300`;
  const icon = <img src="/soop-logo.svg" alt="SOOP 방송국" className={`${large ? 'h-8 w-11' : 'h-7 w-10'} block object-contain`} />;
  if (!stationUrl) return <span className={`${baseClass} pointer-events-none border-cyan-200/10 bg-cyan-300/8 opacity-30 grayscale`} title="방송국 링크 준비중">{icon}</span>;
  return <a href={stationUrl} target="_blank" rel="noreferrer" aria-label="SOOP 방송국" title="SOOP 방송국" className={`${baseClass} border-cyan-200/24 bg-cyan-300/12 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] hover:-translate-y-0.5 hover:scale-[1.05] hover:border-cyan-100/48 hover:bg-cyan-300/18`}>{icon}</a>;
}
...
function CrewCard({ crew }) {
  ...
  return <section className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(14,18,27,0.98),rgba(6,8,13,0.99))] p-5 shadow-[0_20px_54px_rgba(0,0,0,0.32)] transition duration-300 hover:border-white/16 hover:shadow-[0_26px_70px_rgba(0,0,0,0.38)]">
    ...
    <div className="relative mt-5 grid gap-4 lg:grid-cols-[190px_1fr]">
      <div className="rounded-[26px] border border-amber-200/18 bg-[linear-gradient(180deg,rgba(245,158,11,0.12),rgba(255,255,255,0.025))] p-4 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] transition duration-300 hover:border-amber-100/28 hover:bg-amber-200/10">
