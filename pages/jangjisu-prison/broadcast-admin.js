...
export default function BroadcastAdminPage() {
  ...
  function handleLogin(event) {
    ...
    if (password.trim() !== '032359') {
      setMessage('비밀번호가 맞지 않습니다.');
      return;
    }
    localStorage.setItem(ADMIN_STORAGE_KEY, password.trim());
    setAuthorized(true);
    setMessage('');
  }
  ...
  return (
    <>
      ...
      <PrisonPageChrome wide>
        <section className="mx-auto w-full max-w-[1600px] rounded-[30px] bg-[radial-gradient(circle_at_top,rgba(20,184,166,0.12),transparent_28%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_24px_80px_rgba(0,0,0,0.32)] sm:p-8">
          <div className="mb-7">
          ...
          {!authorized ? (
            ...
            <div>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-[22px] bg-white/[0.035] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                <div className="text-sm font-black text-white/64">{payload?.monthLabel || '이번 달'} · {broadcasts.length}개 다시보기</div>
