...
function parseMonthFromLabel(label) {
  ...
  if (!m) { const n = new Date(); return { year: n.getFullYear(), month: n.getMonth() + 1 }; }
...
function CalendarPreview() {
  ...
  const cells = useMemo(() => buildCalendarCells(mainSchedule), [mainSchedule]);
  const { year, month } = useMemo(() => parseMonthFromLabel(mainSchedule.monthLabel), [mainSchedule.monthLabel]);
  const today = new Date();
  const schedules = useMemo(() => (mainSchedule.items || []).filter((item) => !item.empty && String(item.title || '').trim()).map((item) => ({ day: item.dayNumber, member: '장지수', title: item.title })), [mainSchedule]);
  const visible = selectedMember === '전체보기' ? schedules : schedules.filter((item) => item.member === selectedMember);
  const byDay = useMemo(() => {
    const map = new Map();
    visible.forEach((item) => { const list = map.get(item.day) || []; list.push(item); map.set(item.day, list); });
    return map;
  }, [visible]);
  return (
    <section id="schedule" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8">
      <div className="rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)] sm:p-7">
        ...
        <div className="mb-5 flex flex-wrap gap-2 rounded-[24px] border border-white/8 bg-[#05101d] p-3">
          ...
          {SCHEDULE_MEMBERS.map((m) => <button key={m.nickname} onClick={() => setSelectedMember(m.nickname)} className={`rounded-full border px-5 py-2.5 text-[15px] font-black transition ${selectedMember === m.nickname ? 'border-amber-200/32 bg-amber-300/12 text-white shadow-[0_0_18px_rgba(245,158,11,0.10)]' : 'border-white/10 bg-white/[0.06] text-white/72 hover:bg-white/10'}`}>{m.nickname}</button>)}
        </div>
        <div className="rounded-[28px] border border-white/10 bg-[#05101d] p-4 sm:p-5">
          <div className="mb-4 grid grid-cols-7 gap-3 text-center text-[15px] font-black text-white/58">{['일','월','화','수','목','금','토'].map((d,i)=><div key={d} className={i===0?'text-[#ff8e8e]':i===6?'text-[#89b4ff]':''}>{d}</div>)}</div>
          <div className="grid grid-cols-7 gap-3">{cells.map((cell, i) => {
            if (!cell) return <div key={`e-${i}`} className={`${selectedMember === '전체보기' ? 'min-h-[160px]' : 'min-h-[132px]'} rounded-[22px] border border-white/5 bg-white/[0.02]`} />;
