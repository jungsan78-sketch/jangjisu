...
function isRecentUpload(item) { const time = new Date(item?.publishedAt || '').getTime(); return Number.isFinite(time) && Date.now() - time < 24 * 60 * 60 * 1000; }
function hasRecentUpload(items = []) { return items.some(isRecentUpload); }
function shareYoutubeState(scope, payload) { if (typeof window === 'undefined') return; window.__SOU_YOUTUBE_STATE__ = { ...(window.__SOU_YOUTUBE_STATE__ || {}), [scope]: payload }; window.dispatchEvent(new CustomEvent('sou-youtube-loaded', { detail: { scope } })); }
...
function SectionTitle({ title, logo }) { return <div className="mb-5 flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-amber-200/18 bg-amber-200/10 text-lg text-amber-100">{logo}</span><h3 className="text-[28px] font-black tracking-tight text-white sm:text-[34px]">{title}</h3></div>; }

function parseMonthFromLabel(label) { const m = String(label || '').match(/(\d{4})년\s*(\d{1,2})월/); if (!m) { const n = new Date(); return { year: n.getFullYear(), month: n.getMonth() + 1 }; } return { year: Number(m[1]), month: Number(m[2]) }; }
function buildCalendarCells(schedule) { const { year, month } = parseMonthFromLabel(schedule.monthLabel); const days = new Date(year, month, 0).getDate(); const lead = new Date(year, month - 1, 1).getDay(); const total = Math.ceil((lead + days) / 7) * 7; const map = new Map((schedule.items || []).map((item) => [Number(item.dayNumber), item])); return Array.from({ length: total }, (_, i) => { const day = i - lead + 1; if (day < 1 || day > days) return null; return map.get(day) || { dayNumber: day, title: '', empty: true }; }); }
...
function CalendarPreview() {
  ...
  const [youoneulSchedule, setYouoneulSchedule] = useState({ monthLabel: '', items: [], loaded: false });

  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
        const [mainRes, youoneulRes] = await Promise.all([
          fetch('/api/schedule'),
          fetch('/api/youoneul-schedule'),
        ]);
        const [mainJson, youoneulJson] = await Promise.all([mainRes.json(), youoneulRes.json()]);
        if (!mounted) return;
        setMainSchedule({ monthLabel: mainJson.monthLabel || '', items: Array.isArray(mainJson.items) ? mainJson.items : [], loaded: true });
        setYouoneulSchedule({ monthLabel: youoneulJson.monthLabel || '', items: Array.isArray(youoneulJson.items) ? youoneulJson.items : [], loaded: true });
      } catch {
        if (!mounted) return;
        setMainSchedule({ monthLabel: '', items: [], loaded: true });
        setYouoneulSchedule({ monthLabel: '', items: [], loaded: true });
      }
    };
  ...
  const calendarSchedule = useMemo(() => {
    const monthLabel = mainSchedule.monthLabel || youoneulSchedule.monthLabel || '';
    const itemMap = new Map();
    [...(mainSchedule.items || []), ...(youoneulSchedule.items || [])].forEach((item) => {
      if (!item?.dayNumber) return;
      if (!itemMap.has(item.dayNumber)) {
        itemMap.set(item.dayNumber, item);
      }
    });
    return { monthLabel, items: Array.from(itemMap.values()) };
  }, [mainSchedule, youoneulSchedule]);
  ...
  const schedules = useMemo(() => ([
    ...(mainSchedule.items || []).filter((item) => !item.empty && String(item.title || '').trim()).map((item) => ({ day: item.dayNumber, member: '장지수', title: item.title })),
    ...(youoneulSchedule.items || []).filter((item) => !item.empty && String(item.title || '').trim()).map((item) => ({ day: item.dayNumber, member: '유오늘', title: item.title })),
  ]), [mainSchedule, youoneulSchedule]);
  const visible = selectedMember === '전체보기' ? schedules : schedules.filter((item) => item.member === selectedMember);
  const byDay = useMemo(() => { const map = new Map(); visible.forEach((item) => { const list = map.get(item.day) || []; list.push(item); map.set(item.day, list); }); return map; }, [visible]);
  const isLoaded = mainSchedule.loaded && youoneulSchedule.loaded;
...
function NoticeSection() { return <section id="notice" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8"><SectionTitle title="공지" logo="📢" /><div className="rounded-[24px] border border-white/10 bg-[#0b0f17] p-6 text-white/65">점검중</div></section>; }
