...
function PlatformButton({ href, type }) {
  const srcMap = { youtube: '/youtube-logo.svg', cafe: '/naver-cafe-logo.png', soop: '/soop-logo.png' };
  ...
  const baseClass = `inline-flex h-12 w-14 items-center justify-center rounded-2xl border bg-black/18 px-2 transition duration-300 hover:-translate-y-0.5 hover:scale-[1.04] ${toneClass}`;
  const icon = <img src={srcMap[type]} alt={labelMap[type]} className="max-h-8 max-w-[42px] object-contain" />;
  if (!href) return <span className={`${baseClass} pointer-events-none opacity-22`} title={`${labelMap[type]} 준비중`}>{icon}</span>;
