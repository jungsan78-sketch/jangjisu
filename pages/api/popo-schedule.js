...
const SHEET_ID = '1bZwCSbYQ_8g-SmDFf0cWo6Nh6Ysyo1pCmrRhNXJGzpI';
const SHEET_GID = '167079174';
...
function emptyCurrentMonthPayload(currentMonth, fetchedUrl = '') {
  return {
    ...
    message: '포포 일정 시트에서 현재 월 데이터를 찾지 못했습니다.',
  };
}
...
export default async function handler(req, res) {
  ...
  const cacheKey = makeMonthlyScheduleCacheKey('schedule:popo', new Date());
  ...
  try {
    ...
    return res.status(200).json({ ok: false, sourceUrl: SHEET_URL, monthLabel: currentMonth.monthLabel, items: [], message: '포포 일정 데이터를 불러오지 못했습니다.', fetchedAt: new Date().toISOString(), cache: 'unavailable' });
  }
}
