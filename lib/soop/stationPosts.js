...
function pickPostFromPayload(payload, stationId) {
  ...
  walk(payload, (list) => {
    list.forEach((item) => {
      ...
      const title = String(item.title || item.subject || item.post_title || item.bbs_title || item.notice_title || item.szTitle || '').trim();
      const postNo = String(item.post_no || item.bbs_no || item.title_no || item.seq || item.no || '').trim();
      if (!title || title.length < 2) return;
      if (/방송국|즐겨찾기|애청자|구독/.test(title) && !postNo) return;
      candidates.push({
        title,
        postNo,
        viewCount: toNumber(item.view_cnt || item.read_cnt || item.hit || item.total_view_cnt),
        okCount: toNumber(item.ok_cnt || item.up_cnt || item.recommend_cnt || item.total_ok_cnt),
        commentCount: toNumber(item.comment_cnt || item.memo_cnt || item.reply_cnt),
        createdAt: String(item.reg_date || item.created_at || item.write_date || item.date || '').trim(),
      });
    });
  });
  const post = candidates.find((item) => item.postNo) || candidates[0];
  if (!post) return null;
  return {
    stationId,
    title: post.title,
    url: post.postNo ? `https://www.sooplive.com/station/${stationId}/post/${post.postNo}` : `https://www.sooplive.com/station/${stationId}`,
    viewCount: post.viewCount,
    okCount: post.okCount,
...
export async function fetchStationPostsPayload() {
  const posts = await mapLimit(PRISON_MEMBERS, fetchMemberPost);
  const normalized = {};
  posts.filter(Boolean).forEach((post) => { normalized[post.stationId] = post; });
  return { posts: normalized, fetchedAt: new Date().toISOString(), source: 'soop_station_main_data' };
}
