...
const CONCURRENCY = 3;
const TARGETED_HTML_FALLBACK_IDS = new Set(['bxroong', 'isq1158', 'mini1212', 'ddikku0714']);
...
function normalizePost(item, stationId) {
  const title = firstString(item.title_name, item.titleName, item.title, item.subject, item.post_title, item.postTitle, item.bbs_title, item.bbsTitle, item.notice_title, item.noticeTitle, item.szTitle, item.display?.title, item.display?.subject, item.board?.title, item.board?.subject, item.article?.title, item.article?.subject);
  const postNo = firstString(item.title_no, item.titleNo, item.post_no, item.postNo, item.bbs_no, item.bbsNo, item.seq, item.no, item.article_no, item.articleNo, item.notice_no, item.noticeNo, item.id);
  ...
  const createdAt = firstString(item.reg_date, item.regDate, item.created_at, item.createdAt, item.write_date, item.writeDate, item.wdate, item.wDate, item.date, item.created, item.regDt, item.reg_dt, item.display?.created_at, item.display?.createdAt, item.article?.created_at, item.article?.createdAt);

  return {
    stationId,
    stationNo: firstString(item.station_no, item.stationNo, KNOWN_STATION_NO[stationId]),
    bbsNo: firstString(item.bbs_no, item.bbsNo, item.board_no, item.boardNo),
    postNo,
    title: decodeText(title),
    summary: decodeText(firstString(contentValue, item.summary, item.text_content, item.textContent, item.memo, item.preview, item.article?.summary, item.article?.content)),
    boardName: firstString(item.display?.bbs_name, item.display?.bbsName, item.bbs_name, item.bbsName, item.board_name, item.boardName),
    writerId: firstString(item.user_id, item.userId, item.writer_id, item.writerId, item.user?.id, item.user?.user_id, item.writer?.id, item.writer?.user_id),
    writerNick: firstString(item.user_nick, item.userNick, item.writer_nick, item.writerNick, item.user?.nick, item.user?.nickname, item.writer?.nick, item.writer?.nickname),
...
function collectPostsFromPayload(payload, stationId) {
  const candidates = [];
  const directLists = [payload?.posts, payload?.data, payload?.DATA, payload?.list, payload?.result?.posts, payload?.result?.data, payload?.result?.list, payload?.data?.list, payload?.DATA?.list, payload?.data?.posts, payload?.DATA?.posts, payload?.board?.list, payload?.notice?.list].filter(Array.isArray);
...
function collectPostsFromHtml(html, stationId) {
  const candidates = [];
  ...
  while ((match = postLinkRegex.exec(text))) {
    ...
    const chunk = decodeText(text.slice(start, end));
    const titleMatch = chunk.match(/(?:title_name|titleName|title|subject|og:title)["'=:\s]+([^"'<>|]{2,120})/i)
      || chunk.match(/>([^<>]{2,80})<\/a>/i);
    const title = decodeText(titleMatch?.[1] || '최근 공지 확인하기');
    if (!title || /SOOP|숲|방송국|장지수용소 팬메이드/i.test(title)) continue;
    candidates.push(normalizePost({ title, post_no: postNo, user_id: stationId, station_no: KNOWN_STATION_NO[stationId] }, stationId));
  }

  const jsonTitleRegex = /["'](?:title_name|titleName|title|subject)["']\s*:\s*["']([^"']{2,120})["'][\s\S]{0,500}?["'](?:title_no|titleNo|post_no|postNo|bbs_no|bbsNo|id)["']\s*:\s*["']?(\d+)/gi;
  while ((match = jsonTitleRegex.exec(text))) {
    const title = decodeText(match[1]);
    const postNo = match[2];
    if (!title || !postNo || seen.has(postNo)) continue;
    seen.add(postNo);
    candidates.push(normalizePost({ title, post_no: postNo, user_id: stationId, station_no: KNOWN_STATION_NO[stationId] }, stationId));
  }
...
async function fetchHtmlPost(stationId) {
  if (!TARGETED_HTML_FALLBACK_IDS.has(stationId)) return null;
  const urls = [`https://www.sooplive.com/station/${stationId}`, `https://www.sooplive.com/station/${stationId}/board`, `https://bj.afreecatv.com/${stationId}/posts`, `https://bj.afreecatv.com/${stationId}/board`];
  ...
  for (const url of urls) {
    try {
      ...
      if (!response.ok) continue;
      const post = pickLatestPost(collectPostsFromHtml(await response.text(), stationId), stationId);
      if (post) return { ...post, writerId: stationId, source: `soop_html_fallback:${new URL(url).hostname}` };
    } catch {}
  }
  return null;
}
...
function buildBoardCandidateUrls(stationId, stationNo = '') {
  ...
  return [
    ...
    `https://api-channel.sooplive.com/v1.1/channel/${stationId}/home/section/board`,
    `https://api-channel.sooplive.com/v1.1/channel/${stationId}/home/section/notice`,
    `https://bjapi.sooplive.co.kr/api/${stationId}/station/board?${params.toString()}`,
    `https://bjapi.sooplive.co.kr/api/${stationId}/station/posts?${params.toString()}`,
    `https://bjapi.sooplive.co.kr/api/${stationId}/station/notice?${params.toString()}`,
    `https://bjapi.afreecatv.com/api/${stationId}/station/board?${params.toString()}`,
    `https://bjapi.afreecatv.com/api/${stationId}/station/posts?${params.toString()}`,
    `https://bjapi.afreecatv.com/api/${stationId}/station/notice?${params.toString()}`,
...
async function fetchBoardPost(stationId, stationNo) {
  ...
  for (const url of urls) {
    try {
      const response = await timeoutFetch(url, {
        headers: { Accept: 'application/json, text/plain, */*', Referer: `https://www.sooplive.com/station/${stationId}/board`, 'User-Agent': 'Mozilla/5.0' },
        cache: 'no-store',
      });
      if (!response.ok) continue;
      const post = pickLatestPost(collectPostsFromPayload(parseJson(await response.text()), stationId), stationId);
      if (post?.writerId === stationId || !post?.writerId) return { ...post, writerId: post.writerId || stationId, source: `soop_board_api:${new URL(url).hostname}` };
      if (post) return { ...post, source: `soop_board_api_unfiltered:${new URL(url).hostname}` };
    } catch {}
  }
  return fetchHtmlPost(stationId);
}
...
async function fetchMainDataPost(stationId, stationNo) {
  ...
  try {
    const response = await timeoutFetch(MAIN_DATA_URL, {
      method: 'POST',
      headers: { Accept: 'application/json, text/plain, */*', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', Referer: `https://www.sooplive.com/station/${stationId}`, 'User-Agent': 'Mozilla/5.0' },
...
function buildPostDebug(member, stationId, stationNo, boardPost, fallbackPost, selectedPost) {
  return {
    ...
    board: boardPost ? { source: boardPost.source, writerId: boardPost.writerId || '', writerMatchesStation: boardPost.writerId === stationId, title: boardPost.title || '', createdAt: boardPost.createdAt || '' } : null,
    fallback: fallbackPost ? { source: fallbackPost.source, writerId: fallbackPost.writerId || '', writerMatchesStation: fallbackPost.writerId === stationId, title: fallbackPost.title || '', createdAt: fallbackPost.createdAt || '' } : null,
    selected: selectedPost ? { source: selectedPost.source, writerId: selectedPost.writerId || '', writerMatchesStation: selectedPost.writerId === stationId, title: selectedPost.title || '', createdAt: selectedPost.createdAt || '' } : null,
...
async function fetchMemberPost(member) {
  ...
  if (!stationId) {
    return { post: null, debug: { nickname: member.nickname, stationId: '', stationNo: '', hasStationNo: false, stationNoSource: 'missing', board: null, fallback: null, selected: null, reason: 'station_id_not_found' } };
  }
  ...
  const selectedPost = boardPost?.writerId === stationId ? boardPost : fallbackPost?.writerId === stationId ? fallbackPost : boardPost || fallbackPost || null;
