...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-2';
...
function buildPostDebug(member, stationId, stationNo, requestDebug, selectedPost) {
  const debug = {
    ...
    board: selectedPost ? {
      ...
      writerStationId: selectedPost.writerStationId || '',
      writerStationNo: selectedPost.writerStationNo || '',
    ...
    reason: selectedPost ? 'matched_strict_own_post' : (stationNo || KNOWN_STATION_NO[stationId]) ? 'no_strict_own_post_from_primary_endpoint' : 'station_no_not_found',
  };
...
export async function fetchStationPostsPayload(options = {}) {
  ...
  return {
    ...
    source: 'soop_primary_post_endpoint_strict_own_posts_only',
    debug: {
      stationPostsRuntimeMarker: STATION_POSTS_RUNTIME_MARKER,
      subrequestStrategy: 'strict_own_post_only_no_viewer_post_fallback',
