...
function normalizePost(item, stationId) {
  ...
  return {
    stationId,
    stationNo: firstString(item.station_no, item.stationNo),
    bbsNo: firstString(item.bbs_no, item.bbsNo, item.board_no, item.boardNo),
    postNo,
    title,
    summary: firstString(contentValue, item.summary, item.text_content, item.textContent, item.memo, item.preview, item.article?.summary, item.article?.content),
...
async function fetchMainDataPost(stationId, stationNo) {
  ...
  try {
    const response = await timeoutFetch(MAIN_DATA_URL, {
      ...
      body: new URLSearchParams({ bj: stationId, station_no: stationNo, szBjId: stationId, nStationNo: stationNo }).toString(),
      cache: 'no-store',
    });
...
function buildPostDebug(member, stationId, stationNo, boardPost, fallbackPost, selectedPost) {
  return {
    ...
    stationId,
    stationNo: stationNo || '',
    hasStationNo: Boolean(stationNo),
    board: boardPost ? {
    ...
    reason: selectedPost ? 'matched' : stationNo ? 'no_post_from_board_or_main_data' : 'station_no_not_found',
  };
}
...
async function fetchMemberPost(member) {
  ...
  if (!stationId) {
    return {
      ...
      debug: {
        ...
        fallback: null,
        selected: null,
        reason: 'station_id_not_found',
      },
    };
  }
  const stationNo = await getStationNo(stationId);
  const boardPost = await fetchBoardPost(stationId, stationNo);
  const fallbackPost = boardPost?.writerId === stationId ? null : await fetchMainDataPost(stationId, stationNo);
  ...
  return {
    post: selectedPost,
    debug: buildPostDebug(member, stationId, stationNo, boardPost, fallbackPost, selectedPost),
  };
}
