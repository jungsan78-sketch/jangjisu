...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-1';
...
function normalizePost(item, stationId) {
  ...
  return {
    stationId,
    stationNo: firstString(item.stationNo, item.station_no, item.member?.stationId, item.member?.station_id, KNOWN_STATION_NO[stationId]),
    ...
    writerId: firstString(item.userId, item.user_id, item.writerId, item.writer_id, item.bjId, item.bj_id, item.member?.userId, item.member?.user_id, item.member?.stationId, item.member?.station_id, item.user?.id, item.user?.user_id, item.writer?.id, item.writer?.user_id),
...
function isOwnPost(post, stationId) {
  ...
  if (!writerId && !stationNo) return true;
  if (writerId && writerId === stationId) return true;
  if (stationNo && knownStationNo && stationNo === knownStationNo) return true;
  return false;
}

function pickLatestPost(posts, stationId) {
  const scoped = posts.filter((item) => isOwnPost(item, stationId));
...
function buildPostDebug(member, stationId, stationNo, requestDebug, selectedPost) {
  const debug = {
    ...
    board: selectedPost ? {
      ...
      writerMatchesStation: isOwnPost(selectedPost, stationId),
