...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-3';
const HOME_SECTION_FIRST_IDS = new Set(['ayanesena', 'bxroong', 'isq1158', 'mini1212', 'ddikku0714']);
...
async function fetchPostEndpoint(stationId, url, source, refererPath = '') {
  ...
  try {
    const response = await timeoutFetch(url, {
      headers: { Accept: 'application/json, text/plain, */*', Referer: `https://www.sooplive.com/station/${stationId}${refererPath}`, 'User-Agent': 'Mozilla/5.0' },
      cache: 'no-store',
    });
...
function buildPostRequests(stationId) {
  ...
  if (HOME_SECTION_FIRST_IDS.has(stationId)) {
    requests.push({ url: `https://api-channel.sooplive.com/v1.1/channel/${stationId}/home/section/post`, source: 'soop_home_section_post', refererPath: '', debugKey: 'homeSection' });
  }
...
async function fetchBoardPost(stationId) {
  ...
  for (const request of buildPostRequests(stationId)) {
    ...
    if (selected && request.debugKey === 'homeSection') {
      return { post: { ...selected, writerId: selected.writerId || stationId, source: selected.source || request.source }, requestDebug: { key: 'requestAttempts', value: attempts } };
    }
  }
  const selected = pickLatestPost(collectedPosts, stationId);
  return { post: selected ? { ...selected, writerId: selected.writerId || stationId, source: selected.source || 'soop_board_api_multi_page' } : null, requestDebug: { key: 'requestAttempts', value: attempts } };
}
...
function buildPostDebug(member, stationId, stationNo, requestDebug, selectedPost) {
  const debug = {
    ...
    reason: selectedPost ? 'matched_strict_own_post' : (stationNo || KNOWN_STATION_NO[stationId]) ? 'no_strict_own_post_from_multi_page_endpoints' : 'station_no_not_found',
  };
...
export async function fetchStationPostsPayload(options = {}) {
  ...
  return {
    ...
    source: 'soop_multi_page_post_endpoint_strict_own_posts_only',
    debug: {
      stationPostsRuntimeMarker: STATION_POSTS_RUNTIME_MARKER,
      subrequestStrategy: 'strict_own_post_only_home_then_board_pages_1_to_3_no_viewer_post_fallback',
