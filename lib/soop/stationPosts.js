...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-5';
...
function buildPostDebug(member, stationId, stationNo, requestDebug, selectedPost) {
  const debug = {
    ...
    reason: selectedPost ? 'matched_latest_strict_own_post_across_all_endpoints' : (stationNo || KNOWN_STATION_NO[stationId]) ? 'no_strict_own_post_from_home_and_multi_page_endpoints' : 'station_no_not_found',
  };
...
export async function fetchStationPostsPayload(options = {}) {
  ...
  return {
    ...
    source: 'soop_all_post_endpoints_latest_strict_own_posts_only',
    debug: {
      stationPostsRuntimeMarker: STATION_POSTS_RUNTIME_MARKER,
      subrequestStrategy: 'strict_own_post_only_collect_home_and_board_pages_1_to_3_then_pick_latest_no_viewer_post_fallback',
