...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-7';
...
function textFromUnknown(value, depth = 0) {
  ...
  if (typeof value === 'object') {
    const preferredKeys = ['plain_text', 'plainText', 'summary', 'text', 'contents', 'content', 'html', 'body', 'memo', 'description', 'message', 'value'];
...
function normalizePost(item, stationId) {
  ...
  const summary = firstText(item.contents, item.content, contentValue, item.summary, item.textContent, item.text_content, item.memo, item.preview, item.article?.summary, item.article?.content).slice(0, 220);
