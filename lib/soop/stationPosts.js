...
function normalizePost(item, stationId) {
  ...
  return {
    ...
    createdAt: String(item.reg_date || item.regDate || item.created_at || item.createdAt || item.write_date || item.writeDate || item.date || '').trim(),
...
function pickPostFromPayload(payload, stationId) {
  ...
  const ownPosts = unique.filter((item) => item.writerId === stationId);
  return ownPosts[0] || unique.find((item) => item.postNo) || unique[0] || null;
}
