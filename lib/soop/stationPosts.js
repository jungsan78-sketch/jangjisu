...
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-8';
...
function normalizePost(item, stationId, forcedSource = '') {
  ...
  const postNo = firstString(item.titleNo, item.title_no, item.postNo, item.post_no, item.postId, item.post_id, item.bbsNo, item.bbs_no, item.seq, item.no, item.articleNo, item.article_no, item.noticeNo, item.notice_no, item.id);
...
async function fetchWardenLatestPost() {
  const attempts = [];
  const collectedPosts = [];
  for (let page = 1; page <= CHAPI_BOARD_PAGE_LIMIT; page += 1) {
    const result = await fetchPostEndpoint(WARDEN_STATION_ID, buildChapiBoardUrl(WARDEN_STATION_ID, page), `warden_notice_chapi_board:p${page}`, '/board');
    attempts.push(result.debug);
    collectedPosts.push(...result.posts);
  }
  const selected = pickLatestPost(collectedPosts, WARDEN_STATION_ID);
  return { post: selected ? { ...selected, writerId: selected.writerId || WARDEN_STATION_ID, source: selected.source || 'warden_notice_chapi_board' } : null, attempts };
}
...
function buildPostDebug(member, stationId, stationNo, requestDebug, selectedPost) {
  const debug = {
    ...
    board: selectedPost ? { source: selectedPost.source, writerId: selectedPost.writerId || '', writerMatchesStation: isOwnPost(selectedPost, stationId), title: selectedPost.title || '', createdAt: selectedPost.createdAt || '' } : null,
    selected: selectedPost ? { source: selectedPost.source, writerId: selectedPost.writerId || '', writerMatchesStation: isOwnPost(selectedPost, stationId), title: selectedPost.title || '', createdAt: selectedPost.createdAt || '' } : null,
...
export async function fetchStationPostsPayload(options = {}) {
  ...
  let wardenNoticeSync = { attempted: true, replaced: false, selectedTitle: '', selectedCreatedAt: '', reason: '' };
  try {
    const { post: wardenPost, attempts } = await fetchWardenLatestPost();
    const currentWarden = normalized[WARDEN_STATION_ID];
    if (wardenPost && (!currentWarden || Number(wardenPost.createdAtMs || 0) >= Number(currentWarden.createdAtMs || 0))) {
      normalized[WARDEN_STATION_ID] = { ...wardenPost, source: `${wardenPost.source}_synced_for_recent_posts` };
      wardenNoticeSync = { attempted: true, replaced: true, selectedTitle: wardenPost.title, selectedCreatedAt: wardenPost.createdAt, reason: 'warden_notice_source_newer_or_equal', attempts };
    } else {
      wardenNoticeSync = { attempted: true, replaced: false, selectedTitle: wardenPost?.title || '', selectedCreatedAt: wardenPost?.createdAt || '', reason: 'current_warden_post_is_newer_or_notice_missing', attempts };
    }
  } catch (error) {
    wardenNoticeSync = { attempted: true, replaced: false, selectedTitle: '', selectedCreatedAt: '', reason: error?.message || 'warden_notice_sync_failed' };
  }
  ...
  return {
    ...
    debug: {
      stationPostsRuntimeMarker: STATION_POSTS_RUNTIME_MARKER,
      subrequestStrategy: 'strict_own_post_collect_home_chapi_board_pick_latest_plus_warden_notice_source_sync',
