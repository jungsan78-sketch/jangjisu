...
async function fetchHomeSectionPost(stationId) {
  try {
    ...
    const posts = collectPostsFromPayload(parseJson(await response.text()), stationId);
    const post = pickLatestPost(posts, stationId);
    return post ? { ...post, writerId: post.writerId || stationId, source: 'soop_home_section_post' } : null;
  } catch {
    return null;
  }
}
...
async function fetchBoardPost(stationId, stationNo) {
  if (HOME_SECTION_FIRST_IDS.has(stationId)) {
    const homePost = await fetchHomeSectionPost(stationId);
    if (homePost?.writerId === stationId) return homePost;
...
async function fetchMemberPost(member) {
  ...
  if (!stationId) {
    return {
      post: null,
      debug: { nickname: member.nickname, stationId: '', stationNo: '', hasStationNo: false, stationNoSource: 'missing', board: null, fallback: null, selected: null, reason: 'station_id_not_found' },
    };
  }
