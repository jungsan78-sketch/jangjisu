...
function normalizePost(item, stationId) {
  ...
  return {
    ...
    postNo,
    title,
    summary: String(item.content?.summary || item.summary || item.text_content || item.textContent || '').trim(),
    boardName: String(item.display?.bbs_name || item.bbs_name || '').trim(),
    writerId: String(item.user_id || item.userId || item.writer_id || '').trim(),
    writerNick: String(item.user_nick || item.userNick || item.writer_nick || '').trim(),
...
function pickPostFromPayload(payload, stationId) {
  ...
  const ownPosts = candidates.filter((item) => item.writerId === stationId);
  return ownPosts[0] || candidates.find((item) => item.postNo) || candidates[0] || null;
}
...
async function fetchBoardPost(stationId, stationNo) {
  ...
  for (const url of urls) {
    try {
      ...
      if (!response.ok) continue;
      const post = pickPostFromPayload(parseJson(await response.text()), stationId);
...
export async function fetchStationPostsPayload() {
  ...
  return {
    ...
    source: 'soop_board_api_all_members',
    debug: {
