...
const CONCURRENCY = 2;
const STATION_POSTS_RUNTIME_MARKER = 'test2-station-posts-lib-20260428-10';
...
function extractListCandidates(payload) {
  return [
    payload?.data?.list,
    payload?.data?.items,
    payload?.data?.posts,
...
function collectPostsFromPayload(payload, stationId) {
  const candidates = [];
  const directLists = extractListCandidates(payload);
...
export async function fetchStationPostsPayload(options = {}) {
  ...
  return {
    ...
    debug: {
      stationPostsRuntimeMarker: STATION_POSTS_RUNTIME_MARKER,
      subrequestStrategy: 'strict_own_post_collect_home_and_chapi_pick_latest_without_404_board_or_warden_sync_aligned_chapi_parser',
