...
async function fetchBroadListMetadata(memberList) {
  ...
  if (!clientId) {
    return { itemsByUserId: {}, available: false, warning: 'SOOP_CLIENT_ID is not configured' };
  }
  ...
  Object.entries(SOOP_BROAD_LIST_PAGE_LIMITS).forEach(([orderType, maxPages]) => {
    for (let pageNo = 1; pageNo <= maxPages; pageNo += 1) {
      requests.push({ orderType, pageNo });
    }
  });

  const results = await Promise.allSettled(
    requests.map(({ orderType, pageNo }) => fetchBroadListPage(clientId, pageNo, orderType)),
  );
...
async function fetchLiveStatusesFast() {
  ...
  if (broadListResult?.available) {
    ...
    const liveCount = Object.values(statuses).filter((status) => status.isLive).length;
    const shouldPlayFallback = process.env.SOOP_LIVE_PLAY_FALLBACK === '1';

    if (!shouldPlayFallback) {
      return {
        statuses,
        source: 'soop_openapi_broad_list',
        fetchedAt: new Date().toISOString(),
        debug: {
          openApi: {
    ...
    return {
      statuses,
      source: liveCount ? 'soop_openapi_broad_list_play_fallback' : 'soop_play_fallback',
      fetchedAt: new Date().toISOString(),
      debug: {
        openApi: {
          matched: Object.keys(broadListResult.itemsByUserId || {}).length,
  ...
  crawledStatuses.forEach((status) => {
    statuses[status.nickname] = status;
  });
  ...
  try {
    ...
    if (envJson) {
      return {
        ...
        warning: `SOOP live fast path fallback used: ${error.message}`,
      };
    }
