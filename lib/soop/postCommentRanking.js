...
function buildOfficialCommentUrl(stationId, postNo, page = 1, commentNo = '0') {
  const params = new URLSearchParams({
    page: String(page),
    orderBy: 'reg_date',
    cCommentNo: String(commentNo || '0'),
  });
  return `https://api-channel.sooplive.com/v1.1/channel/${stationId}/post/${postNo}/comment?${params.toString()}`;
}

async function requestCommentPage(stationId, postNo, page, commentNo, referer) {
  const url = buildOfficialCommentUrl(stationId, postNo, page, commentNo);
...
function getNextCommentNo(result) {
  const payload = result?.payload || {};
  const direct = firstText(
    payload.next_comment_no,
    payload.nextCommentNo,
    payload.c_comment_no,
    payload.cCommentNo,
    payload.data?.next_comment_no,
    payload.data?.nextCommentNo,
    payload.data?.c_comment_no,
    payload.data?.cCommentNo,
  );
  if (direct && direct !== '0') return direct;
  const lastCommentNo = result?.comments?.[result.comments.length - 1]?.commentNo || '';
  return /^\d+$/.test(lastCommentNo) ? lastCommentNo : '';
}
...
export async function fetchSoopPostUpRanking(postUrl, options = {}) {
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    let result;
    try {
      result = await requestCommentPage(parsed.stationId, parsed.postNo, page, cursor, parsed.url);
    } catch (error) {
      attempts.push({ page, cursor, status: 0, ok: false, error: error?.message || String(error) });
      if (page === 1) {
    ...
    const nextCursor = getNextCommentNo(result);
  ...
  return {
    ...
    source: 'soop_api_channel_comment',
    fetchedAt: new Date().toISOString(),
    ...(options.debug ? { debug: { attempts, sample: comments.slice(0, 3) } } : {}),
  };
}
