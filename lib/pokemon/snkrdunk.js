...
export async function searchSnkrdunkProducts(query, limit = 12) {
  ...
  const idsToHydrate = Array.from(map.keys()).slice(0, Math.min(limit, 12));
  ...
  hydrated.forEach((result, index) => {
    ...
    const displayJpy = product.jpy || product.latestTradeJpy || null;
    ...
    map.set(idsToHydrate[index], {
      ...
      image: product.image,
      jpy: displayJpy,
      jpyText: formatYen(displayJpy),
      currentJpy: product.jpy,
      currentJpyText: product.jpyText,
