...
export function normalizeScheduleText(value) {
  const normalized = stripDecorativePrefix(
    String(value || '')
      .replace(/\s*\/\s*/g, ' / ')
      .replace(/\s*\\\s*/g, ' \\ ')
      .replace(/\s{2,}/g, ' ')
...
function isIgnoredCalendarText(value) {
  const text = normalizeScheduleText(value);
  if (!text) return true;
  if (/^\d{1,2}$/u.test(text)) return true;
  if (/^(Sun|Mon|Tue|Wed|Thu|Fri|Sat)$/i.test(text)) return true;
  if (/^(일|월|화|수|목|금|토)$/u.test(text)) return true;
  if (/^(비고|정기휴방)$/u.test(text)) return true;
  if (/^20\d{2}년\s*\d{1,2}월/u.test(text)) return true;
  if (/^20\d{2}년\s*\d{1,2}월\s*\/\s*20\d{2}년\s*\d{1,2}월/u.test(text)) return true;
  if (/20\d{2}년\s*\d{1,2}월/u.test(text) && !/[가-힣]{2,}/u.test(text.replace(/20\d{2}년\s*\d{1,2}월/gu, ''))) return true;
  if (/일정표!?$/u.test(text)) return true;
  return false;
}
...
function collectDetailTexts(detailRows, startColumnIndex, endColumnIndexExclusive) {
  return detailRows.flatMap((detailRow) => {
    const texts = [];
    for (let columnIndex = startColumnIndex; columnIndex < endColumnIndexExclusive; columnIndex += 1) {
      const normalized = normalizeScheduleText(detailRow[columnIndex] || '');
      if (!normalized || isIgnoredCalendarText(normalized)) continue;
...
function collectLooseDetailTexts(detailRows, startColumnIndex, endColumnIndexExclusive) {
  const texts = [];
...
function isNextMonthCarryoverDateRow(row, lastIncludedDay, daysInMonth) {
  if (!isLooseDateRow(row)) return false;
  const numericCells = row
    .map((cell) => Number(String(cell || '').trim()))
    .filter((value) => Number.isFinite(value) && value >= 1 && value <= 31);

  if (!numericCells.length) return false;
  const includesCurrentMonthEnd = numericCells.some((value) => value >= Math.max(28, daysInMonth - 2));
  const includesNextMonthStart = numericCells.some((value) => value >= 1 && value <= 7 && value <= lastIncludedDay);
  return includesCurrentMonthEnd && includesNextMonthStart;
}
...
export function parseScheduleRows(rows, targetYear, targetMonth) {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    ...
    while (
      ...
      !isDateRow(rows[nextRowIndex]) &&
      !isNextMonthCarryoverDateRow(rows[nextRowIndex], lastIncludedDay, daysInMonth)
    ) {
      detailRows.push(rows[nextRowIndex]);
      nextRowIndex += 1;
    }

    activeDays.forEach(({ day, columnIndex }, dayIndex) => {
      const nextColumnIndex = activeDays[dayIndex + 1]?.columnIndex ?? row.length;
      const detailTexts = collectDetailTexts(detailRows, columnIndex, nextColumnIndex);
...
export function parseLooseCalendarRows(rows, targetYear, targetMonth) {
  const daysInMonth = new Date(targetYear, targetMonth, 0).getDate();
  const itemsMap = new Map();
  let lastIncludedDay = 0;
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    const row = rows[rowIndex];
    if (!isLooseDateRow(row)) continue;
    ...
    while (
      nextRowIndex < rows.length &&
      !isLooseDateRow(rows[nextRowIndex]) &&
      !isNextMonthCarryoverDateRow(rows[nextRowIndex], lastIncludedDay, daysInMonth)
    ) {
      detailRows.push(rows[nextRowIndex]);
      nextRowIndex += 1;
    ...
    activeDays.forEach(({ day, columnIndex }, dayIndex) => {
      const nextColumnIndex = activeDays[dayIndex + 1]?.columnIndex ?? row.length;
      const detailTexts = collectLooseDetailTexts(detailRows, columnIndex, nextColumnIndex);
      const dateObject = new Date(targetYear, targetMonth - 1, day);
...
export function parseScheduleListRows(rows, targetYear, targetMonth) {
  ...
  rows.forEach((row) => {
    ...
    const detailTexts = row
      ...
      .filter((cell) => cell && !isIgnoredCalendarText(cell));
