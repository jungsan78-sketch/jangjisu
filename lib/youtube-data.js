...
const SHORTS_HALL_WINDOW_MS = 1000 * 60 * 60 * 24 * 30;
...
export async function fetchMainYoutubePayload(options = {}) {
  ...
  const shorts = pickShorts(mainUploads).slice(0, 30);
...
export async function fetchPrisonYoutubePayload(options = {}) {
  ...
  try {
    ...
    return {
      ...
      shorts: merged.filter((item) => item.type === 'shorts').slice(0, 60),
      missingKey: false,
      fetchedAt: new Date().toISOString(),
    };
  } catch {
...
function viewsNumber(video) {
  return Number(String(video?.views || video?.viewCount || '0').replace(/,/g, '')) || 0;
}

function isWithinShortsHallWindow(video, now = Date.now()) {
  const published = new Date(video?.publishedAt || '').getTime();
  return Number.isFinite(published) && now - published <= SHORTS_HALL_WINDOW_MS;
}

function normalizeHallShort(video, fallbackMember) {
  if (!video) return null;
  return {
    id: video.id,
    title: video.title || '',
    member: video.member || fallbackMember || '',
    thumbnail: video.thumbnail || '',
    publishedAt: video.publishedAt || '',
    publishedAtText: video.publishedAtText || formatPublishedAt(video.publishedAt || ''),
    views: String(viewsNumber(video)),
    viewsText: video.viewsText || formatViews(viewsNumber(video)),
    durationText: video.durationText || '',
    url: video.url || (video.id ? `https://www.youtube.com/shorts/${video.id}` : ''),
  };
}

export function buildShortsHallOfFame(mainPayload = {}, prisonPayload = {}) {
  const jangjisu = (mainPayload.shorts || [])
    .filter((video) => isWithinShortsHallWindow(video))
    .map((video) => normalizeHallShort({ ...video, member: '장지수' }, '장지수'))
    .filter(Boolean)
    .sort((a, b) => viewsNumber(b) - viewsNumber(a))[0] || null;

  const memberTop = (prisonPayload.shorts || [])
    .filter((video) => isWithinShortsHallWindow(video))
    .map((video) => normalizeHallShort(video, video.member))
    .filter(Boolean)
    .filter((video) => video.member !== '장지수')
    .sort((a, b) => viewsNumber(b) - viewsNumber(a));

  return {
    ok: Boolean(jangjisu || memberTop.length),
    windowDays: 30,
    refreshLabel: '6시간마다 갱신',
    fetchedAt: new Date().toISOString(),
    slots: {
      memberTop1: memberTop[0] || null,
      jangjisu,
      memberTop2: memberTop[1] || null,
    },
  };
}
