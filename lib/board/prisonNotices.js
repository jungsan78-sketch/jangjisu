...
function extractCandidates(html, member) {
  const pattern = new RegExp(`<a[^>]+href=["']([^"']*\/station\/${member.stationId}\/(?:post|board)[^"']*)["'][^>]*>([\\s\\S]*?)<\/a>`, 'gi');
  const items = [];
  let match;
  while ((match = pattern.exec(html)) !== null) {
    const title = stripTags(match[2]);
    if (!title || title.length < 2) continue;
    const context = html.slice(Math.max(0, match.index - 300), Math.min(html.length, match.index + match[0].length + 1000));
    if (!new RegExp(member.nickname, 'i').test(context) && !new RegExp(member.stationId, 'i').test(context)) continue;
    ...
    items.push({
      ...
      createdAt,
      summary: stripTags(context).slice(0, 140),
    });
  }
  return items;
}
...
async function fetchMemberBoard(member) {
  const response = await fetch(`https://www.sooplive.com/station/${member.stationId}/board`, { cache: 'no-store' });
