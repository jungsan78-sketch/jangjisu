...
import { PRISON_NOTICE_OVERRIDES } from '../../data/prisonNoticeOverrides';
...
export async function fetchRecentPrisonNotices() {
  ...
  const manualOverrides = PRISON_NOTICE_OVERRIDES.filter((notice) => isRecent(notice.createdAt));

  return results
    .flat()
    .concat(manualOverrides)
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
