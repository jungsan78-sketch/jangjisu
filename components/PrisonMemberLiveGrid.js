...
function LiveGridSkeleton({ failed = false }) {
  ...
  return <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5" aria-busy={!failed}>{skeletonItems.map((item) => <div key={item} className="min-h-[260px]"><div className="aspect-video rounded-[24px] bg-white/[0.035]" /><div className="mt-3 flex gap-3"><div className="h-11 w-11 rounded-full bg-white/[0.055]" /><div className="flex-1"><div className="h-5 w-28 rounded-full bg-white/[0.055]" /><div className="mt-3 h-4 w-full rounded-full bg-white/[0.04]" /><div className="mt-2 h-4 w-2/3 rounded-full bg-white/[0.035]" /></div></div></div>)}{failed ? <div className="col-span-full rounded-[22px] bg-amber-300/8 px-5 py-4 text-sm font-black text-amber-100/80">라이브/게시글 상태를 확인하는 중입니다.</div> : null}</div>;
}
