...
const SETTINGS_KEY = 'sou-soop-funding-persistent-settings-v1';
const EVENTS_KEY = 'sou-soop-funding-persistent-events-v1';
...
function makeEventMemo(events, settings) {
...
function Button({ active, className = '', ...props }) {
  return <button {...props} className={`rounded-2xl border px-3 py-2.5 text-xs font-black transition ${active ? 'border-cyan-200/40 bg-cyan-300/16 text-cyan-50' : 'border-white/10 bg-white/[0.045] text-white/60 hover:bg-white/[0.07]'} ${className}`} />;
}

export default function SoopFundingMemoPersistent() {
  ...
  useEffect(() => {
    ...
    const savedEvents = readJson(EVENTS_KEY, []).filter((e) => e?.id && e?.name && num(e.amount) > 0).slice(0, MAX_EVENTS);
    setSettings(savedSettings);
    setEvents(savedEvents);
    settingsRef.current = savedSettings;
    seenRef.current = new Set(savedEvents.map((e) => e.id));
    setHydrated(true);
  }, []);
  ...
  const collectorScript = useMemo(() => {
    if (!appUrl) return '';
    return `(function(){
  const targetOrigin=${JSON.stringify(typeof window === 'undefined' ? '' : new URL(appUrl).origin)};
  ...
  const memo = useMemo(() => makeEventMemo(events, settings), [events, settings]);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#05070c] text-white">
      ...
      <main className="relative mx-auto max-w-[1780px] px-5 py-5 lg:px-8"><section className="grid gap-4 xl:grid-cols-[410px_minmax(0,1fr)_410px] 2xl:grid-cols-[430px_minmax(0,1fr)_430px]">
        ...
        <div className="space-y-4">
          <Card title="후원자 순위 TOP 10" caption="후원 개수는 닉네임 기준으로 누적됩니다." className="xl:h-[520px]"><div className="mb-3 grid grid-cols-2 gap-3"><StatBox label="총 펀딩" value={fmt(total)} tone="emerald" /><StatBox label="유효 건수" value={validEvents.length} /></div><div className="max-h-[370px] space-y-2 overflow-auto pr-1">{ranking.length ? ranking.map((item, index) => { const rank = index + 1; return <div key={item.name} className={`rounded-[20px] border px-3.5 py-3 transition ${rank === 1 ? 'border-yellow-200/28 bg-yellow-300/10 shadow-[0_0_34px_rgba(250,204,21,0.10)]' : 'border-white/10 bg-[#07101a]'}`}><div className="flex items-center gap-3"><span className="text-lg">{rank === 1 ? '👑' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : rank}</span><div className="min-w-0 flex-1"><div className="truncate text-sm font-black text-white">{item.name}</div><div className="mt-1 text-[11px] font-bold text-white/36">후원 {item.count}회</div></div><div key={item.amount} className="animate-[numberPop_420ms_ease-out] text-right text-base font-black text-cyan-50">{fmt(item.amount)}</div></div></div>; }) : <div className="rounded-[24px] border border-dashed border-white/12 bg-black/18 p-8 text-center"><div className="text-lg font-black text-white/70">순위 대기 중</div><div className="mt-2 text-sm font-semibold text-white/40">후원이 들어오면 TOP 10이 표시됩니다.</div></div>}</div></Card>
