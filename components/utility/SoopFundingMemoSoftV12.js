...
const RECONNECT_DELAYS = [1500, 2500, 4000, 6000, 9000, 12000, 16000, 20000, 30000];
...
function isTokenExpiring(token) {
  ...
  return Date.now() + TOKEN_REFRESH_MARGIN_MS >= token.expiresAt;
}
...
function StatusPill({ status }) {
  const tone = ['connected', 'received', 'authorized', 'refreshing'].includes(status) ? 'bg-emerald-300 text-emerald-950' : ['closed', 'reconnecting', 'connecting'].includes(status) ? 'bg-amber-300 text-amber-950' : ['error'].includes(status) ? 'bg-rose-300 text-rose-950' : 'bg-slate-400 text-slate-950';
...
function HealthLine({ label, ok, value }) {
  return <div className="flex items-center justify-between gap-3 rounded-xl bg-black/20 px-3 py-2"><span className="text-white/45">{label}</span><span className={ok ? 'text-emerald-100' : 'text-rose-100'}>{value || (ok ? '있음' : '없음')}</span></div>;
}

export default function SoopFundingMemoSoftV12() {
  ...
  const refreshToken = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      if (!silent) {
        setStatus('error');
        setStatusText(`${text} SOOP 로그인을 다시 진행해주세요.`);
      }
      throw error;
    } finally {
  ...
  const addEvent = useCallback((event) => {
    ...
    setEvents((prev) => [{ ...event, id: safeId }, ...prev].slice(0, MAX_EVENTS));
    return true;
  }, []);
  ...
  const connectSdk = useCallback(async ({ reconnect = false, forceRefresh = false } = {}) => {
    ...
    if (!clientId) {
      setStatus('error');
      setStatusText('NEXT_PUBLIC_SOOP_CLIENT_ID 환경변수가 없습니다.');
      setLastSdkError('NEXT_PUBLIC_SOOP_CLIENT_ID 없음');
      return;
    }
    if (!clientSecret) {
      setStatus('error');
      setStatusText('NEXT_PUBLIC_SOOP_CLIENT_SECRET 환경변수가 없습니다. Cloudflare 환경변수 추가 후 재배포해주세요.');
      setLastSdkError('NEXT_PUBLIC_SOOP_CLIENT_SECRET 없음');
      return;
    }
    if (!sdkUrl) {
      setStatus('error');
      setStatusText('NEXT_PUBLIC_SOOP_CHAT_SDK_URL 환경변수가 없습니다.');
      setLastSdkError('NEXT_PUBLIC_SOOP_CHAT_SDK_URL 없음');
      return;
    }
    ...
    try {
      ...
      if (typeof sdk.handleError === 'function') sdk.handleError((code, msg) => {
        if (manualStopRef.current) return;
        const detail = `${code || ''} ${msg || ''}`.trim() || 'SDK 오류 원문 없음';
        setLastDisconnectedAt(nowText());
        setLastSdkError(detail);
        setStatus('error');
        setStatusText(`연결 오류: ${detail}. 자동 복구를 계속 시도합니다.`);
        sdkRef.current = null;
      ...
      if (String(detail).includes("reading 'on'") || String(detail).includes('reading \'on\'')) {
        reconnectAttemptsRef.current = Math.max(reconnectAttemptsRef.current, 8);
      }
      scheduleReconnect('연결 실패', { forceRefresh: true });
    } finally {
      isConnectingRef.current = false;
    }
  }, [clientId, clientSecret, ensureFreshToken, handleSdkPayload, safeDisconnectSdk, sdkLoaded, sdkUrl]);
  ...
  useEffect(() => {
    ...
    autoConnectTimerRef.current = window.setTimeout(() => connectSdk(), 500);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#07101f,#091827_42%,#06101f)] text-white">
      ...
      <header className="sticky top-0 z-40 bg-[#06101e]/78 backdrop-blur-2xl shadow-[0_10px_32px_rgba(0,0,0,0.22)]"><div className="mx-auto flex max-w-[1760px] items-center justify-between gap-4 px-5 py-4 lg:px-8"><div className="flex items-center gap-3"><a href="/utility" className="rounded-full bg-white/[0.06] px-4 py-2 text-sm font-black text-white/80 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)] transition hover:bg-white/[0.10]">← 유틸리티</a><h1 className="text-xl font-black sm:text-2xl">펀딩 자동메모장</h1></div><a href="/" className="rounded-full bg-cyan-300/12 px-4 py-2 text-xs font-black text-cyan-50 shadow-[inset_0_0_0_1px_rgba(103,232,249,0.16)]">장지수용소</a></div></header>
      <main className="relative mx-auto max-w-[1760px] px-5 py-6 lg:px-8"><section className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)_420px]">
        <div className="space-y-5">
          <Panel title="방송 연결" desc="SOOP 로그인 후 방송 채팅 연결을 자동으로 시도합니다."><div className="mb-4 rounded-[28px] bg-[#052235]/80 p-4 shadow-[inset_0_0_0_1px_rgba(103,232,249,0.09)]"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-cyan-300/13"><img src="/logos/SOOP.png" alt="SOOP" className="max-h-7 max-w-10 object-contain" /></div><div><div className="text-sm font-black text-white">SOOP 파트너 연결</div><div className="mt-1 text-xs font-bold text-cyan-50/58">문서 기준 init + connect 성공 후 연결됨 표시</div></div></div><StatusPill status={status} /></div><div className="mt-4 rounded-[22px] bg-[#061421]/88 p-4 text-sm font-bold leading-6 text-white/76 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.055)]">{statusText}</div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] font-black text-white/48"><span>마지막 수신: {lastReceivedAt}</span><span>마지막 끊김: {lastDisconnectedAt}</span></div></div><div className="grid gap-2"><Button onClick={startLogin}>SOOP 로그인 / 재로그인</Button><Button tone="rose" onClick={disconnectSdk} wide>연결 중지</Button></div><div className="mt-3 grid gap-1 rounded-2xl bg-black/20 p-3 text-[11px] font-black">{debugRows.map((row) => <HealthLine key={row.label} {...row} />)}<div className="rounded-xl bg-black/20 px-3 py-2 text-white/55">마지막 SDK 오류: <span className="text-amber-100">{lastSdkError}</span></div></div></Panel>
          <Panel title="기준 설정" desc="몇 개부터 복붙 결과에 넣을지 정합니다."><label className="block rounded-[26px] bg-cyan-300/[0.075] p-4 shadow-[inset_0_0_0_1px_rgba(103,232,249,0.11)]"><div className="text-sm font-black text-cyan-50">유효개수</div><div className="mt-1 text-xs font-semibold text-white/52">복붙 결과에 반영할 최소 단위</div><div className="mt-3"><Field type="number" min="1" value={settings.validCount} onChange={(e) => setSettings((p) => ({ ...p, validCount: Math.max(1, toNumber(e.target.value, 1000)) }))} /></div></label></Panel>
          <Panel title="수동 추가" desc="누락된 후원을 직접 입력합니다."><div className="grid gap-3"><Field value={manual.name} onChange={(e) => setManual((p) => ({ ...p, name: e.target.value }))} placeholder="후원 닉네임" /><Field value={manual.message} onChange={(e) => setManual((p) => ({ ...p, message: e.target.value }))} placeholder="후원 메시지" /><div className="flex gap-2"><Field type="number" value={manual.amount} onChange={(e) => setManual((p) => ({ ...p, amount: e.target.value }))} placeholder="후원 개수" /><Button onClick={addManual}>추가</Button></div></div></Panel>
        </div>
        <div className="space-y-5"><Panel title="핀볼 복붙 결과" desc="후원 닉네임 또는 메시지 기준으로 바로 복사합니다." right={<div className="flex gap-2"><Button onClick={copyMemo} disabled={!memo}>{copied ? '복사됨' : '복사'}</Button><Button tone="rose" onClick={resetEvents}>초기화</Button></div>}><div className="mb-4 grid gap-2 rounded-[24px] bg-[#061421]/70 p-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.055)] md:grid-cols-2"><ChoiceButton active={settings.outputMode !== 'message'} onClick={() => setSettings((p) => ({ ...p, outputMode: 'nickname' }))}>후원 닉네임으로 복사</ChoiceButton><ChoiceButton active={settings.outputMode === 'message'} onClick={() => setSettings((p) => ({ ...p, outputMode: 'message' }))}>후원 메시지로 복사</ChoiceButton></div><textarea value={memo} readOnly placeholder="아직 복붙할 후원 데이터가 없습니다." className="h-[390px] w-full resize-none rounded-[30px] bg-white px-5 py-5 text-[20px] font-black leading-9 text-gray-900 outline-none shadow-[inset_0_0_0_1px_rgba(103,232,249,0.10)] placeholder:text-gray-500" /><div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-white/54"><span className="rounded-full bg-cyan-300/8 px-3 py-1.5">유효개수 {formatNumber(validCount)}개</span><span className="rounded-full bg-cyan-300/8 px-3 py-1.5">기록 {events.length}건</span><span className="rounded-full bg-cyan-300/8 px-3 py-1.5">유효 {validEvents.length}건</span></div></Panel></div>
        <div className="space-y-5"><Panel title="후원자 순위 TOP 10" desc="후원 개수 합산 기준입니다."><div className="mb-3 grid grid-cols-2 gap-3"><div className="rounded-[22px] bg-emerald-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">총 펀딩</div><div className="mt-1 text-2xl font-black">{formatNumber(total)}</div></div><div className="rounded-[22px] bg-cyan-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">유효 건수</div><div className="mt-1 text-2xl font-black">{validEvents.length}</div></div></div><div className="max-h-[460px] space-y-2 overflow-auto pr-1">{ranking.length ? ranking.map((item, index) => <div key={item.name} className="flex items-center justify-between gap-3 rounded-[20px] border border-white/10 bg-[#07101a] px-3.5 py-3"><div className="min-w-0"><div className="truncate text-sm font-black text-white">{index + 1}. {item.name}</div><div className="mt-1 text-[11px] font-bold text-white/36">후원 {item.count}회</div></div><div className="text-base font-black text-cyan-50">{formatNumber(item.amount)}</div></div>) : <div className="rounded-[24px] border border-dashed border-white/12 bg-black/18 p-8 text-center text-white/50">순위 대기 중</div>}</div></Panel></div>
      </section></main>
    </div>
  );
}
