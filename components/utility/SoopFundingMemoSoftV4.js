...
const SETTINGS_KEY = 'sou-soop-funding-soft-v4-settings';
const EVENTS_KEY = 'sou-soop-funding-soft-v4-events';
...
function Panel({ title, desc, right, children, className = '' }) {
  return (
    <section className={`relative overflow-hidden rounded-[32px] bg-[linear-gradient(145deg,rgba(19,35,56,0.92),rgba(8,17,31,0.94))] p-5 shadow-[0_24px_70px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.06)] ring-1 ring-white/[0.055] ${className}`}>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(56,189,248,0.12),transparent_34%),radial-gradient(circle_at_95%_15%,rgba(110,231,183,0.08),transparent_28%)]" />
      <div className="relative mb-4 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-[21px] font-black tracking-[-0.02em] text-white">{title}</h2>
          {desc ? <p className="mt-1 text-sm font-bold leading-6 text-slate-300/80">{desc}</p> : null}
        </div>
        {right}
      </div>
...
function Field(props) {
  return <input {...props} className="w-full rounded-[22px] bg-[#061727]/92 px-4 py-4 text-[18px] font-black text-white outline-none shadow-[inset_0_0_0_1px_rgba(125,211,252,0.13)] placeholder:text-white/32 transition focus:bg-[#082033] focus:shadow-[inset_0_0_0_1px_rgba(103,232,249,0.45),0_0_0_4px_rgba(34,211,238,0.09)]" />;
}

function Button({ children, tone = 'cyan', wide = false, ...props }) {
  const toneClass = tone === 'green'
    ? 'bg-emerald-400/18 text-emerald-50 shadow-[inset_0_0_0_1px_rgba(110,231,183,0.26)] hover:bg-emerald-400/25'
    : tone === 'rose'
      ? 'bg-rose-400/14 text-rose-50 shadow-[inset_0_0_0_1px_rgba(251,113,133,0.24)] hover:bg-rose-400/22'
      : tone === 'yellow'
        ? 'bg-amber-300/13 text-amber-100 shadow-[inset_0_0_0_1px_rgba(252,211,77,0.22)] hover:bg-amber-300/20'
        : 'bg-cyan-300/14 text-cyan-50 shadow-[inset_0_0_0_1px_rgba(103,232,249,0.25)] hover:bg-cyan-300/22';
  return <button type="button" {...props} className={`${wide ? 'w-full' : ''} rounded-[22px] px-4 py-4 text-sm font-black transition hover:-translate-y-0.5 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-45 ${toneClass}`}>{children}</button>;
}
...
export default function SoopFundingMemoSoftV4() {
      ...
      <main className="relative mx-auto max-w-[1760px] px-5 py-6 lg:px-8">
        <section className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)_420px]">
          <div className="space-y-5">
            <Panel title="방송 연결" desc="켜두면 후원을 기다립니다. 끊기면 자동복구를 시도합니다.">
              ...
              <div className="grid gap-2"><div className="grid grid-cols-2 gap-2"><Button onClick={startLogin}><img src="/logos/SOOP.png" alt="" className="mr-2 inline h-4 w-auto" />SOOP 로그인</Button><Button tone="green" onClick={token ? () => connectSdk() : startLogin} disabled={isConnecting}>{isConnecting ? '연결 중...' : '방송 채팅 연결'}</Button></div><Button tone="rose" onClick={disconnectSdk} wide>연결 중지</Button></div>
          ...
          <div className="space-y-5">
            <Panel title="핀볼 복붙 결과" desc="후원 닉네임 또는 메시지 기준으로 바로 복사합니다." right={<div className="flex gap-2"><Button onClick={copyMemo} disabled={!memo}>{copied ? '복사됨' : '복사'}</Button><Button tone="rose" onClick={resetEvents}>초기화</Button></div>}>
              <div className="mb-4 grid gap-2 rounded-[24px] bg-[#061421]/70 p-3 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.055)] md:grid-cols-2"><Button tone={settings.outputMode !== 'message' ? 'green' : 'cyan'} onClick={() => setSettings((p) => ({ ...p, outputMode: 'nickname' }))}>후원 닉네임으로 복사</Button><Button tone={settings.outputMode === 'message' ? 'green' : 'cyan'} onClick={() => setSettings((p) => ({ ...p, outputMode: 'message' }))}>후원 메시지로 복사</Button></div>
