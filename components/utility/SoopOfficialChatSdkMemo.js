...
const SETTINGS_KEY = 'sou-soop-official-sdk-settings-v1';
const EVENTS_KEY = 'sou-soop-official-sdk-events-v1';
const TOKEN_KEY = 'sou-soop-official-sdk-token-v1';
...
export default function SoopOfficialChatSdkMemo() {
  ...
  const connectSdk = useCallback(() => {
    ...
    if (!sdkLoaded || typeof window === 'undefined' || !window.ChatSDK) {
      setStatus('error');
      setStatusText('Chat SDK가 아직 로드되지 않았습니다. SDK URL 설정을 확인하세요.');
      return;
    }
    try {
      const clientId = process.env.NEXT_PUBLIC_SOOP_CLIENT_ID || '';
      const clientSecretPlaceholder = '';
      const sdk = new window.ChatSDK(clientId, clientSecretPlaceholder);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#05070c] text-white">
      {chatSdkUrl ? <Script src={chatSdkUrl} strategy="afterInteractive" onLoad={() => setSdkLoaded(true)} onError={() => setStatusText('Chat SDK 스크립트 로드 실패')} /> : null}
