...
function StatusPill({ status }) {
  const tone = ['connected', 'received', 'ready'].includes(status)
    ? 'bg-emerald-300 text-emerald-950'
    : ['connecting'].includes(status)
    ...
    <div className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#07101f,#091827_42%,#06101f)] text-white">
      ...
      <header className="sticky top-0 z-40 bg-[#06101e]/78 backdrop-blur-2xl shadow-[0_10px_32px_rgba(0,0,0,0.22)]">
        <div className="mx-auto flex max-w-[1760px] items-center justify-between gap-4 px-5 py-4 lg:px-8">
          <div className="flex items-center gap-3"><a href="/utility" className="rounded-full bg-white/[0.06] px-4 py-2 text-sm font-black text-white/80">← 유틸리티</a><h1 className="text-xl font-black sm:text-2xl">SOOP 펀딩 New</h1></div>
          ...
          <div className="space-y-5">
            <Panel title="자동수신 설정" desc="SOOP 로그인 없이 스트리머 아이디 기준으로 후원 이벤트만 수신합니다.">
              <div className="mb-4 rounded-[28px] bg-[#052235]/80 p-4 shadow-[inset_0_0_0_1px_rgba(103,232,249,0.09)]">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] font-black text-white/48"><span>마지막 수신: {lastReceivedAt}</span><span>최대 기록: {formatNumber(MAX_EVENTS)}건</span></div>
              </div>
              <div className="grid gap-3">
                <label className="block"><div className="mb-2 text-sm font-black text-cyan-50">스트리머 아이디</div><Field value={streamerId} onChange={(event) => setStreamerId(event.target.value)} placeholder="예: go0117" /></label>
                <label className="block"><div className="mb-2 text-sm font-black text-cyan-50">유효개수</div><Field type="number" min="1" value={validCount} onChange={(event) => setValidCount(event.target.value)} /></label>
                <Button onClick={start}>자동수신 시작</Button>
                <Button tone="rose" onClick={stop} wide>자동수신 중지</Button>
                <Button tone="rose" onClick={reset} wide>기록 초기화</Button>
              </div>
            </Panel>
          </div>
          ...
          <div className="space-y-5">
            <Panel title="후원 수신 목록" desc="유효개수 이상 후원만 표시합니다.">
              <div className="mb-4 grid grid-cols-3 gap-3">
                <div className="rounded-[22px] bg-cyan-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">전체 건수</div><div className="mt-1 text-2xl font-black">{formatNumber(events.length)}</div></div>
                <div className="rounded-[22px] bg-emerald-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">유효 건수</div><div className="mt-1 text-2xl font-black">{formatNumber(validEvents.length)}</div></div>
                <div className="rounded-[22px] bg-emerald-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">유효 합계</div><div className="mt-1 text-2xl font-black">{formatNumber(validTotal)}</div></div>
              </div>
              <div className="max-h-[620px] space-y-2 overflow-auto pr-1">
                {validEvents.length ? validEvents.map((event) => <div key={event.id} className="flex items-center justify-between gap-3 rounded-[20px] border border-white/10 bg-[#07101a] px-4 py-4"><div className="min-w-0"><div className="truncate text-lg font-black text-white">{event.name}</div><div className="mt-1 text-[11px] font-bold text-white/36">{event.kind}</div></div><div className="text-2xl font-black text-cyan-50">{formatNumber(event.amount)}개</div></div>) : <div className="rounded-[24px] border border-dashed border-white/12 bg-black/18 p-10 text-center text-white/50">유효 후원 수신 대기 중</div>}
              </div>
            </Panel>
          </div>

          <div className="space-y-5">
            <Panel title="전체 요약" desc="유효개수 미만 후원도 합산에는 보관됩니다.">
              <div className="grid gap-3"><div className="rounded-[22px] bg-cyan-300/10 p-4 text-center"><div className="text-xs font-black text-white/45">총 후원</div><div className="mt-1 text-3xl font-black">{formatNumber(total)}</div></div><div className="rounded-[22px] bg-white/[0.05] p-4 text-sm font-bold leading-7 text-white/60">New 버전은 SOOP 로그인, 토큰, 메시지 복사 없이 후원자 닉네임과 개수만 수신합니다.</div></div>
            </Panel>
          </div>
        </section>
      </main>
