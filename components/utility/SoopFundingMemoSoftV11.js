...
export default function SoopFundingMemoSoftV11() {
  ...
  const refreshToken = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      if (!silent) {
        setStatus('error');
        setStatusText(`${text} 1번 SOOP 로그인을 다시 진행해주세요.`);
      }
      throw error;
    } finally {
  ...
  const connectSdk = useCallback(async ({ reconnect = false, forceRefresh = false } = {}) => {
    ...
    if (!clientId) {
      setStatus('error');
      setStatusText('NEXT_PUBLIC_SOOP_CLIENT_ID 환경변수가 없습니다. Cloudflare 환경변수 추가 후 재배포해주세요.');
    ...
    try {
      ...
      const sdk = new ChatSdk(clientId, '');
  ...
  }, [clientId, ensureFreshToken, handleSdkPayload, safeDisconnectSdk, scheduleReconnect, sdkLoaded, sdkUrl]);
        ...
        <section className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)_420px]">
          <div className="space-y-5">
            <Panel title="방송 연결" desc="SOOP 로그인 후 방송 채팅 연결을 자동으로 시도합니다.">
              ...
              <div className="mt-3 grid gap-1 rounded-2xl bg-black/20 p-3 text-[11px] font-black">
                {debugRows.map((row) => <HealthLine key={row.label} {...row} />)}
                <div className="rounded-xl bg-black/20 px-3 py-2 text-white/55">마지막 SDK 오류: <span className="text-amber-100">{lastSdkError}</span></div>
              </div>
            </Panel>
