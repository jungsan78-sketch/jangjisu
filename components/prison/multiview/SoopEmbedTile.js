...
export default function SoopEmbedTile({ member, status, featured = false, onRemove }) {
  const stationId = getStationId(member, status);
  const playerUrl = stationId ? `https://play.sooplive.com/${encodeURIComponent(stationId)}/embed` : '';
      ...
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between gap-3 bg-gradient-to-b from-black/82 via-black/48 to-transparent px-3 pb-8 pt-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate text-[10px] font-bold text-white/55 sm:text-[11px]">{status?.title || 'SOOP 방송'}</div>
          </div>
        </div>
