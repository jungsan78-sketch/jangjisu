...
const LIVE_STATUS_STORAGE_KEY = 'jangjisou-live-status-cache-v1';
...
function formatViewerCount(value) {
  ...
  if (count >= 10000) return `${(count / 10000).toFixed(count >= 100000 ? 0 : 1).replace(/\.0$/, '')}만명`;
  if (count >= 1000) return `${(count / 1000).toFixed(1).replace(/\.0$/, '')}천명`;
  return `${count}명`;
}

function LiveBadge({ status, isRefreshing }) {
  ...
  if (isRefreshing) {
    return <div className="mt-2 inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-white/[0.04] px-2.5 py-1 text-[10px] font-black tracking-[0.14em] text-white/42 sm:text-[11px]"><span className="h-2 w-2 animate-pulse rounded-full bg-cyan-200/60" />상태 반영중</div>;
  }
  return null;
}

function LiveHoverCard({ member, status, large = false }) {
  if (!status?.isLive) return null;

  return (
    <div className={`pointer-events-none absolute left-1/2 z-20 hidden -translate-x-1/2 overflow-hidden rounded-[24px] border border-white/12 bg-[linear-gradient(180deg,rgba(7,12,22,0.98),rgba(4,8,15,0.98))] shadow-[0_28px_80px_rgba(0,0,0,0.46)] group-hover:block ${large ? 'bottom-[calc(100%+14px)] w-[320px]' : 'bottom-[calc(100%+12px)] w-[280px]'}`}>
      <div className="relative aspect-[16/9] w-full bg-[#09101d]">
        {status.thumbnailUrl ? (
          <img src={status.thumbnailUrl} alt={`${member.nickname} 방송 썸네일`} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.16),transparent_40%),linear-gradient(180deg,#0d1523,#09101d)] text-sm font-bold text-white/45">실시간 썸네일 준비중</div>
        )}
        <div className="absolute inset-x-0 bottom-0 bg-[linear-gradient(180deg,transparent,rgba(0,0,0,0.72))] px-4 pb-3 pt-8">
          <div className="inline-flex items-center rounded-full border border-rose-300/20 bg-black/30 px-3 py-1 text-xs font-black tracking-[0.14em] text-rose-100">LIVE SNAPSHOT</div>
        </div>
      </div>
      <div className="space-y-3 px-4 py-4 text-left sm:px-5">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-sm font-black tracking-[0.08em] text-white/92">{member.nickname}</div>
            <div className="mt-1 text-xs font-semibold text-white/48">실시간 방송 정보</div>
          </div>
          <div className="shrink-0 rounded-full border border-cyan-200/18 bg-cyan-300/10 px-3 py-1.5 text-sm font-black text-cyan-100">{formatViewerCount(status.viewerCount)}</div>
        </div>
        <div className="rounded-[18px] border border-white/8 bg-white/[0.04] px-4 py-3">
          <div className="mb-1 text-[11px] font-black tracking-[0.14em] text-white/42">방송 제목</div>
          <div className="line-clamp-3 text-[14px] font-semibold leading-6 text-white/92">{status.title || '현재 방송중'}</div>
        </div>
      </div>
    </div>
  );
...
function ProfileImage({ member, status, large = false }) {
...
function ProfileCard({ member, status, large = false, isRefreshing = false }) {
  const isLive = status?.isLive === true;
  ...
  return (
    <div className="group relative">
      <LiveHoverCard member={member} status={status} large={large} />
      <Wrapper {...wrapperProps} className={`block rounded-[22px] border ${isLive ? 'border-rose-300/18 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.018)),radial-gradient(circle_at_top,rgba(244,63,94,0.12),transparent_38%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_14px_32px_rgba(0,0,0,0.18),0_0_0_1px_rgba(244,63,94,0.06)]' : 'border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.018))] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_14px_32px_rgba(0,0,0,0.18)]'} p-3 text-center transition hover:-translate-y-1 hover:border-white/18 hover:bg-white/[0.065] sm:rounded-[26px] sm:p-4 ${large ? 'mx-auto w-full max-w-[320px] sm:max-w-[360px]' : ''}`}>
        <ProfileImage member={member} status={status} large={large} />
        <div className={`${large ? 'text-lg sm:text-xl' : 'text-[13px] sm:text-sm'} mt-3 font-black text-white sm:mt-4`}>{member.nickname}</div>
        <LiveBadge status={status} isRefreshing={isRefreshing} />
        {isLive && status?.title ? <div className="mt-2 line-clamp-2 text-[12px] font-semibold leading-5 text-rose-50/92 sm:text-[13px]">{status.title}</div> : null}
...
function writeStoredStatuses(statuses) {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(LIVE_STATUS_STORAGE_KEY, JSON.stringify(statuses || {}));
  } catch {}
}

export default function MemberBoardPreview() {
  const initialStatuses = useMemo(() => readStoredStatuses(), []);
  const [liveStatuses, setLiveStatuses] = useState(initialStatuses);
  const [isRefreshing, setIsRefreshing] = useState(!Object.keys(initialStatuses).length);

  useEffect(() => {
    let mounted = true;
    const loadLiveStatus = async ({ silent = false } = {}) => {
      if (!silent && mounted) setIsRefreshing(true);
      try {
        const res = await fetch('/api/live-status');
        const json = await res.json();
        if (!mounted) return;
        const nextStatuses = json.statuses || {};
        setLiveStatuses(nextStatuses);
        writeStoredStatuses(nextStatuses);
      } catch {
        if (mounted && !Object.keys(liveStatuses).length) setLiveStatuses(readStoredStatuses());
      } finally {
    ...
    loadLiveStatus();
    const timer = setInterval(() => loadLiveStatus({ silent: true }), 60 * 1000);
    return () => {
      mounted = false;
      clearInterval(timer);
    };
  ...
  return (
    <section id="members" className="mt-6 rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.02))] p-4 shadow-[0_22px_60px_rgba(0,0,0,0.28)] sm:mt-8 sm:rounded-[34px] sm:p-6 lg:p-8">
      ...
      <div className="rounded-[24px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(245,158,11,0.10),transparent_30%),linear-gradient(180deg,rgba(13,17,25,0.98),rgba(5,7,11,0.99))] p-4 shadow-[0_22px_55px_rgba(0,0,0,0.34)] sm:rounded-[32px] sm:p-5 lg:p-7">
        ...
        <ProfileCard member={WARDEN} status={liveStatuses[WARDEN.nickname]} large isRefreshing={isRefreshing} />
        ...
        <div className="mt-6 border-t border-white/10 pt-6 sm:mt-8 sm:pt-7">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4 xl:grid-cols-5">
            {PRISON_MEMBERS.map((member) => <ProfileCard key={member.nickname} member={member} status={liveStatuses[member.nickname]} isRefreshing={isRefreshing} />)}
          </div>
        </div>
      </div>
    </section>
