...
function YoutubeTabButton({ label, isActive, onClick, hasNew }) {
  return (
    <button
      onClick={onClick}
      className={`relative inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${
        isActive
      ...
      <main className="relative mx-auto max-w-7xl px-5 py-6 lg:px-8 lg:py-8">
        ...
        <section id="youtube" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8">
          ...
          <div className="mb-6 flex flex-wrap gap-3">
            ...
            <YoutubeTabButton label="풀영상" isActive={activeTab === 'full'} onClick={() => setActiveTab('full')} hasNew={hasNewFull} />
          </div>
        ...
        {isYoutubeDebugMode ? (
          <section className="mt-8 rounded-[32px] border border-yellow-300/20 bg-yellow-500/[0.05] p-6 shadow-xl shadow-black/20 lg:p-8">
            <SectionTitle title="YOUTUBE DEBUG" logo="🛠️" />
            <div className="rounded-[24px] border border-yellow-300/20 bg-[#0b0f17] p-4">
              <pre className="overflow-x-auto whitespace-pre-wrap break-all text-xs leading-6 text-yellow-100/90">{JSON.stringify(youtubeDebug || { message: 'debug data not loaded' }, null, 2)}</pre>
            </div>
          </section>
        ) : null}
      </main>
      ...
      <footer className="mt-16 border-t border-white/10 bg-[linear-gradient(180deg,rgba(0,0,0,0.28),rgba(2,8,18,0.68))]">
        <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
          <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.12),transparent_35%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] px-6 py-10 text-center shadow-[0_14px_40px_rgba(0,0,0,0.18)]">
            ...
            <p className="mt-5 text-xs font-semibold uppercase tracking-[0.28em] text-white/34">ARCHIVED WITH RESPECT</p>
            <p className="mt-4 text-sm italic text-white/56">Fan Archive by 장지수 편집자</p>
            <p className="mt-5 text-xs text-white/30">© 2026 JANGJISOU FAN ARCHIVE</p>
          </div>
        </div>
      </footer>
    </div>
