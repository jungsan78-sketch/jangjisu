  ...
  return (
    <section id="schedule" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8">
      ...
      {!schedule.loaded || !hasItems ? (
        <div className="rounded-[24px] border border-white/10 bg-[#07111f] px-6 py-10 text-sm font-semibold text-white/65">일정 데이터를 불러오는 중이거나 시트 구조를 확인하는 중입니다.</div>
      ) : (
        <>
          <div className="sm:hidden">
            <div className="rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)]">
              ...
              {todayItems.length > 0 ? <div className="space-y-4">{todayItems.map((item) => <ScheduleItem key={`${item.date}-${item.title}`} item={item} />)}</div> : <div className="rounded-[24px] border border-white/10 bg-[#05101d] px-5 py-8 text-sm font-semibold text-white/65">오늘 등록된 일정이 없습니다.</div>}
            </div>
          </div>
          <div className="hidden sm:block rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)] sm:p-7">
            ...
            <div className="rounded-[28px] border border-white/10 bg-[#05101d] p-4 sm:p-5">
              <div className="mb-4 grid grid-cols-7 gap-3 text-center text-sm font-extrabold text-white/60">{weekdayHeaders.map((label, index) => <div key={label} className={index === 0 ? 'text-[#ff8e8e]' : index === 6 ? 'text-[#89b4ff]' : ''}>{label}</div>)}</div>
              <div className="space-y-3">{weeks.map((week, weekIndex) => <div key={`week-${weekIndex}`} className="grid grid-cols-7 gap-3">{week.map((item, weekdayIndex) => <CalendarDayCell key={item ? `${item.date}-${item.title}` : `empty-${weekIndex}-${weekdayIndex}`} item={item} weekdayIndex={weekdayIndex} month={month} />)}</div>)}</div>
            </div>
          </div>
        </>
      )}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#04070d]">
      ...
      <div className="relative flex flex-col items-center">
        <div className="relative h-32 w-32 animate-[spin_8s_linear_infinite] overflow-hidden rounded-full border border-white/10 shadow-[0_0_40px_rgba(59,130,246,0.16)]"><img src="/site-icon.png" alt="SOU" className="h-full w-full object-cover" /></div>
...
export default function JangJisuFanSite() {
  ...
  const [youtube, setYoutube] = useState({ shorts: [], videos: [], full: [], channels: { latest: { url: 'https://www.youtube.com/@jisoujang', videosUrl: 'https://www.youtube.com/@jisoujang/videos', shortsUrl: 'https://www.youtube.com/@jisoujang/shorts' }, full: { url: 'https://www.youtube.com/@jisoujang_full' } } });
      ...
      try {
        ...
        setYoutube({
          ...
          channels: {
            ...
            full: { url: json.channels?.full?.url || 'https://www.youtube.com/@jisoujang_full' },
          },
        });
  ...
  return (
    <div className="min-h-screen bg-[#05070c] text-white">
      <style jsx global>{`html { scroll-behavior: smooth; } @keyframes heroFloat { 0% { transform: translate3d(0, 0, 0) scale(1.02); } 50% { transform: translate3d(0, -8px, 0) scale(1.05); } 100% { transform: translate3d(0, 0, 0) scale(1.02); } } @keyframes shimmerText { 0% { background-position: 0% 50%; } 100% { background-position: 100% 50%; } }`}</style>
      ...
      <main className="relative mx-auto max-w-7xl px-5 py-6 lg:px-8 lg:py-8">
        ...
        <section id="youtube" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8"><SectionTitle title="YOUTUBE" logo="▶" /><div className="mb-6 flex flex-wrap gap-3">{[{ key: 'videos', label: '영상' }, { key: 'shorts', label: '쇼츠' }, { key: 'full', label: '풀영상' }].map((tab) => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${activeTab === tab.key ? 'border border-red-400/40 bg-red-500/20 text-white shadow-[0_0_15px_rgba(255,0,0,0.4)]' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}>{tab.label}</button>)}</div>{activeTab === 'videos' ? <><div className="mb-5 flex items-center justify-between gap-3"><div className="text-[24px] font-extrabold tracking-tight text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.45)] sm:text-[28px]">영상</div></div><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{(youtube?.videos || []).map((video) => <VideoCard key={video.id} video={video} />)}</div></> : null}{activeTab === 'shorts' ? <><div className="mb-5 flex items-center justify-between gap-3"><div className="text-[24px] font-extrabold tracking-tight text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.45)] sm:text-[28px]">쇼츠</div></div><div className="grid grid-cols-2 gap-6 md:grid-cols-3 xl:grid-cols-4">{(youtube?.shorts || []).map((video) => <VideoCard key={video.id} video={video} vertical />)}</div></> : null}{activeTab === 'full' ? <><div className="mb-5 flex items-center justify-between gap-3"><div className="text-[24px] font-extrabold tracking-tight text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.45)] sm:text-[28px]">풀영상</div></div><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{(youtube?.full || []).map((video) => <VideoCard key={video.id} video={video} />)}</div></> : null}</section>
      ...
      <footer className="mt-16 border-t border-white/10 bg-black/30"><div className="mx-auto max-w-7xl px-5 py-10 lg:px-8"><div className="text-center"><div className="text-xs uppercase tracking-[0.35em] text-white/40">JANGJISOU FAN ARCHIVE</div><div className="mt-3 text-2xl font-semibold tracking-[0.18em] text-white">JANGJISOU FAN ARCHIVE</div><p className="mt-4 text-sm leading-7 text-white/60">장지수 팬 아카이브 · 제작 장지수 편집자</p><p className="mt-2 text-sm leading-7 text-white/45">비영리 팬사이트이며, 모든 콘텐츠의 저작권은 원저작권자에게 있습니다.</p><p className="mt-4 text-sm italic text-white/50">— 장지수 편집자 배상</p><p className="mt-2 text-xs text-white/35">© 2026 JANGJISOU FAN ARCHIVE</p></div></div></footer>
    </div>
  );
}
