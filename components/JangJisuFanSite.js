...
function NavChip({ href, label, tone = 'neutral', external = false, icon = '' }) {
  const toneClass = tone === 'green' ? 'border-[#03C75A]/30 bg-[#03C75A]/15 text-[#8df0b6] hover:bg-[#03C75A]/22' : tone === 'blue' ? 'border-[#3b82f6]/30 bg-[#3b82f6]/15 text-[#b8d8ff] hover:bg-[#3b82f6]/22' : tone === 'red' ? 'border-[#ff4e45]/30 bg-[#ff4e45]/15 text-[#ffb2ae] hover:bg-[#ff4e45]/22' : 'border-white/10 bg-white/5 text-white/80 hover:bg-white/10';
  return (
...
export default function JangJisuFanSite() {
  ...
  useEffect(() => { const timer = setTimeout(() => setShowSplash(false), 4200); return () => clearTimeout(timer); }, []);
  useEffect(() => {
    let mounted = true;
