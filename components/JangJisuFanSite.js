  ...
  return (
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      {actionHref && actionLabel ? (
        <a href={actionHref} target="_blank" rel="noreferrer" className="w-fit rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/80 transition hover:bg-white/10">{actionLabel}</a>
      ) : null}
    </div>
  );
}
...
function VideoCard({ video, vertical = false }) {
  const isNew = isRecentUpload(video);
  ...
  return (
    <a href={video.url} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-[26px] border border-white/10 bg-[#0c1018] transition hover:-translate-y-1 hover:border-white/20">
      <div className={`relative overflow-hidden bg-[#121826] ${vertical ? 'aspect-[9/14]' : 'aspect-video'}`}>
        ...
        {isNew ? <div className="absolute right-3 top-3 text-[10px] font-black tracking-[0.08em] text-[#ff9e98] drop-shadow-[0_0_10px_rgba(255,95,95,0.35)]">new</div> : null}
...
function YoutubeTabButton({ label, isActive, onClick, hasNew }) {
  return (
    <button
      onClick={onClick}
      className={`relative inline-flex items-center justify-center rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${isActive ? 'border border-red-400/40 bg-red-500/20 text-white shadow-[0_0_15px_rgba(255,0,0,0.4)]' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}
    >
      <span>{label}</span>
...
function YoutubePanel({ title, items, vertical = false, moreHref, moreLabel }) {
  return (
    <div className="animate-[youtubeTabIn_220ms_ease-out]">
      ...
      <div className={vertical ? 'grid grid-cols-2 gap-6 md:grid-cols-3 xl:grid-cols-4' : 'grid gap-6 md:grid-cols-2 xl:grid-cols-3'}>
        {(items || []).map((video) => <VideoCard key={video.id} video={video} vertical={vertical} />)}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={`rounded-[24px] border p-5 transition ${wrapperClass}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className={`rounded-full border px-3 py-1 text-xs ${badgeClass}`}>{offDay ? '휴방' : isToday ? '오늘 일정' : item.day}</div>
      </div>
...
function buildCalendarWeeks(monthLabel, items) {
  ...
  for (let index = 0; index < totalCells; index += 1) {
    ...
    const item = itemMap.get(dayNumber) || { dayNumber, date: `${month}월 ${dayNumber}일`, day: ['일', '월', '화', '수', '목', '금', '토'][dateObject.getDay()], title: '', empty: true };
    cells.push(item);
  }
  ...
  return (
    <div className={`group relative min-h-[108px] overflow-hidden rounded-[22px] border p-4 transition-all duration-300 ${isToday ? 'border-cyan-300/50 bg-[linear-gradient(180deg,rgba(7,27,46,0.98),rgba(5,12,24,0.98))] shadow-[0_0_0_1px_rgba(103,232,249,0.18),0_0_22px_rgba(34,211,238,0.12)] hover:-translate-y-1 hover:shadow-[0_0_0_1px_rgba(103,232,249,0.24),0_16px_36px_rgba(34,211,238,0.14)]' : offDay ? 'border-orange-300/20 bg-[linear-gradient(180deg,rgba(34,20,7,0.82),rgba(8,14,25,0.98))] hover:-translate-y-1 hover:border-orange-200/30 hover:shadow-[0_14px_30px_rgba(251,146,60,0.10)]' : hasTitle ? 'border-white/10 bg-[linear-gradient(180deg,rgba(11,23,38,0.96),rgba(7,17,31,0.98))] hover:-translate-y-1 hover:border-cyan-300/20 hover:shadow-[0_14px_30px_rgba(56,189,248,0.08)]' : 'border-white/8 bg-[#07111f] hover:border-white/12 hover:bg-[#091729]'}`}>
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className={`text-[15px] font-extrabold ${isSunday ? 'text-[#ff8e8e]' : isSaturday ? 'text-[#89b4ff]' : 'text-white'}`}>{item.dayNumber}</div>
        {isToday ? <span className="rounded-full border border-cyan-300/25 bg-cyan-300/12 px-2 py-1 text-[10px] font-extrabold tracking-[0.18em] text-cyan-100">TODAY</span> : hasTitle ? <span className={`mt-0.5 h-2.5 w-2.5 rounded-full ${offDay ? 'bg-orange-300 shadow-[0_0_12px_rgba(253,186,116,0.55)]' : 'bg-cyan-300 shadow-[0_0_12px_rgba(103,232,249,0.45)]'}`} /> : null}
      </div>
      {hasTitle ? <div className="relative mt-4 text-sm font-semibold leading-6 text-white/92 whitespace-pre-line break-keep">{item.title}</div> : null}
    </div>
  );
}
...
function ScheduleCalendarSection({ schedule }) {
  ...
  return (
    <section id="schedule" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8">
      ...
      {!schedule.loaded || !hasItems ? <div className="rounded-[24px] border border-white/10 bg-[#07111f] px-6 py-10 text-sm font-semibold text-white/65">일정 데이터를 불러오는 중이거나 시트 구조를 확인하는 중입니다.</div> : <>
        <div className="sm:hidden"><div className="rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)]"><div className="mb-5 flex items-center justify-between gap-4"><div className="text-[22px] font-extrabold tracking-tight text-white">오늘 일정</div><div className="text-xs font-bold tracking-[0.28em] text-white/55">{yearText}</div></div>{todayItems.length > 0 ? <div className="space-y-4">{todayItems.map((item) => <ScheduleItem key={`${item.date}-${item.title}`} item={item} />)}</div> : <div className="rounded-[24px] border border-white/10 bg-[#05101d] px-5 py-8 text-sm font-semibold text-white/65">오늘 등록된 일정이 없습니다.</div>}</div></div>
        <div className="hidden sm:block rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)] sm:p-7"><div className="mb-5 flex items-center justify-between gap-4"><div className="text-[24px] font-extrabold tracking-tight text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.45)] sm:text-[30px]">{monthTitle}</div><div className="text-xs font-bold tracking-[0.45em] text-white/40 sm:text-sm">{yearText}</div></div><div className="rounded-[28px] border border-white/10 bg-[#05101d] p-4 sm:p-5"><div className="mb-4 grid grid-cols-7 gap-3 text-center text-sm font-extrabold text-white/60">{weekdayHeaders.map((label, index) => <div key={label} className={index === 0 ? 'text-[#ff8e8e]' : index === 6 ? 'text-[#89b4ff]' : ''}>{label}</div>)}</div><div className="space-y-3">{weeks.map((week, weekIndex) => <div key={`week-${weekIndex}`} className="grid grid-cols-7 gap-3">{week.map((item, weekdayIndex) => <CalendarDayCell key={item ? `${item.date}-${item.title}` : `empty-${weekIndex}-${weekdayIndex}`} item={item} weekdayIndex={weekdayIndex} month={month} />)}</div>)}</div></div></div>
      </>}
    </section>
  );
}

function UtilityCardButton({ active, onClick, title, description }) {
  return (
    <button onClick={onClick} className={`w-full rounded-[24px] border px-5 py-5 text-left transition ${active ? 'border-cyan-300/30 bg-cyan-300/10 shadow-[0_0_0_1px_rgba(103,232,249,0.12),0_18px_36px_rgba(34,211,238,0.08)]' : 'border-white/10 bg-[#0b0f17] hover:border-white/20 hover:bg-[#0f1522]'}`}>
...
function OverwatchRandomPicker() {
  ...
  return (
    <div className="mt-6 rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(8,12,20,0.98),rgba(7,10,18,0.98))] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.22)] lg:p-6">
...
function UtilitySection() {
  const [activeTool, setActiveTool] = useState('ow-random');

  return (
    <section id="utility" className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8">
      <SectionTitle title="UTILITY" logo="🛠️" />
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        <div className="space-y-4">
          <UtilityCardButton active={activeTool === 'ow-random'} onClick={() => setActiveTool('ow-random')} title="오버워치 랜덤뽑기" description="팀 수, 5:5 / 6:6, 잠금 기능까지 포함한 방송용 팀 편성 유틸리티" />
        </div>
        <div>
          {activeTool === 'ow-random' ? <OverwatchRandomPicker /> : null}
        </div>
      </div>
    </section>
  );
...
function SplashScreen() {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#04070d]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(59,130,246,0.18),transparent_35%),radial-gradient(circle_at_center,rgba(6,182,212,0.12),transparent_55%)]" /><div className="relative flex flex-col items-center"><div className="relative h-32 w-32 animate-[spin_8s_linear_infinite] overflow-hidden rounded-full border border-white/10 shadow-[0_0_40px_rgba(59,130,246,0.16)]"><img src="/site-icon.png" alt="SOU" className="h-full w-full object-cover" /></div><div className="mt-8 text-xs uppercase tracking-[0.45em] text-white/45">LOADING ARCHIVE</div><div className="mt-3 text-4xl font-black tracking-[0.26em] text-white">SOU</div></div></div>
  );
}
  ...
  useEffect(() => {
    ...
    const loadSoop = async () => {
      try {
        ...
        if (!mounted) return;
        setData((prev) => ({ ...prev, channel: { ...prev.channel, ...(json.channel || {}) } }));
      } catch {}
    };

    const loadYoutube = async () => {
      try {
        const res = await fetch(debugEnabled ? '/api/youtube?debug=1' : '/api/youtube');
        const json = await res.json();
        if (!mounted) return;
        setYoutube({
          ...
          channels: {
            ...
            full: {
              ...
              videosUrl: json.channels?.full?.videosUrl || 'https://www.youtube.com/@jisoujang_full/videos',
            },
          },
        });
        setYoutubeDebug(json.debug || null);
      } catch {}
    };
    ...
    const loadSchedule = async () => {
      try {
        ...
        const json = await res.json();
        if (!mounted) return;
        setSchedule({ monthLabel: json.monthLabel || '', items: Array.isArray(json.items) ? json.items : [], sourceUrl: json.sourceUrl || '', loaded: true });
      } catch {}
    };
    ...
    return () => { mounted = false; clearInterval(scheduleTimer); };
  }, []);
