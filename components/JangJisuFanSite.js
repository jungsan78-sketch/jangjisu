...
function YoutubePanel({ title, items, vertical = false, moreHref, moreLabel }) {
  return (
    <div className="animate-[youtubeTabIn_220ms_ease-out]">
      ...
      <div className={vertical ? 'grid grid-cols-2 gap-6 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5' : 'grid gap-6 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4'}>{(items || []).map((video) => <VideoCard key={video.id} video={video} vertical={vertical} />)}</div>
    </div>
  );
}
...
function ScheduleItem({ item }) {
  ...
  return <div className={`sou-fan-today-card rounded-[24px] border p-5 transition ${wrapperClass}`}><div className="flex items-center justify-between gap-3"><div className="text-sm font-semibold text-white">{item.date}</div><div className={`rounded-full border px-3 py-1 text-xs ${badgeClass}`}>{offDay ? '휴방' : isToday ? '오늘 일정' : item.day}</div></div><div className="mt-4"><ScheduleSegments title={item.title} offDay={offDay} /></div>{isToday ? <div className="mt-4 inline-flex rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs font-semibold text-cyan-100">TODAY</div> : null}</div>;
}
...
function CalendarDayCell({ item, weekdayIndex, month }) {
  if (!item) return <div className="sou-fan-calendar-empty min-h-[118px] rounded-[22px] border border-white/5 bg-white/[0.02]" />;
...
function ScheduleCalendarSection({ schedule }) {
  ...
  return <section id="schedule" className="mt-8 w-full max-w-none rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-xl shadow-black/20 lg:p-8"><SectionTitle eyebrow={schedule.monthLabel || '이번 일정'} title="장지수 일정" actionHref={schedule.sourceUrl || '#'} actionLabel={schedule.sourceUrl ? '시트 보기' : '일정 보기'} logo="🔵" />{!schedule.loaded || !hasItems ? <div className="rounded-[24px] border border-white/10 bg-[#07111f] px-6 py-10 text-sm font-semibold text-white/65">일정 데이터를 불러오는 중이거나 시트 구조를 확인하는 중입니다.</div> : <><div className="sm:hidden"><div className="sou-fan-calendar-shell rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)]"><div className="mb-5 flex items-center justify-between gap-4"><div className="text-[22px] font-extrabold tracking-tight text-white">오늘 일정</div><div className="text-xs font-bold tracking-[0.28em] text-white/55">{yearText}</div></div>{todayItems.length > 0 ? <div className="space-y-4">{todayItems.map((item) => <ScheduleItem key={`${item.date}-${item.title}`} item={item} />)}</div> : <div className="sou-fan-calendar-empty rounded-[24px] border border-white/10 bg-[#05101d] px-5 py-8 text-sm font-semibold text-white/65">오늘 등록된 일정이 없습니다.</div>}</div></div><div className="sou-fan-calendar-shell hidden w-full max-w-none sm:block rounded-[30px] border border-[#12305c] bg-[radial-gradient(circle_at_top,rgba(22,78,145,0.18),transparent_26%),linear-gradient(180deg,rgba(4,10,22,0.98),rgba(3,9,20,0.98))] p-5 shadow-[0_20px_50px_rgba(0,0,0,0.28)] sm:p-7"><div className="mb-5 flex items-center justify-between gap-4"><div className="text-[24px] font-extrabold tracking-tight text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.45)] sm:text-[30px]">{monthTitle}</div><div className="text-xs font-bold tracking-[0.45em] text-white/40 sm:text-sm">{yearText}</div></div><div className="sou-fan-calendar-grid w-full max-w-none rounded-[28px] border border-white/10 bg-[#05101d] p-4 sm:p-5"><div className="mb-4 grid grid-cols-7 gap-3 text-center text-sm font-extrabold text-white/60">{weekdayHeaders.map((label, index) => <div key={label} className={index === 0 ? 'text-[#ff8e8e]' : index === 6 ? 'text-[#89b4ff]' : ''}>{label}</div>)}</div><div className="space-y-3">{weeks.map((week, weekIndex) => <div key={`week-${weekIndex}`} className="grid grid-cols-7 gap-3">{week.map((item, weekdayIndex) => <CalendarDayCell key={item ? `${item.date}-${item.title}` : `empty-${weekIndex}-${weekdayIndex}`} item={item} weekdayIndex={weekdayIndex} month={month} />)}</div>)}</div></div></div></>}</section>;
}
...
export default function JangJisuFanSite() {
  ...
  useEffect(() => {
    ...
    loadSoop(); loadYoutube(); loadSchedule(); const stopSchedulePolling = startVisibleInterval(loadSchedule, SCHEDULE_REFRESH_INTERVAL_MS); return () => { mounted = false; stopSchedulePolling(); };
  }, []);
